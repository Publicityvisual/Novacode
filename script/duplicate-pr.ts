...
import { createOpencode } from "@novacode-ai/sdk"
...
`)
  ...
  const novacode = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await novacode.client.session.create()
    const result = await novacode.client.session
      .prompt({
