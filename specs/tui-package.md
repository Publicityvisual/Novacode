...
Move the canonical NovaCode terminal application from
`packages/novacode/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/novacode ---\
                      > @novacode-ai/tui -> @novacode-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/novacode`,
`packages/cli`, or `@novacode-ai/core`.

The SDK is the TUI's NovaCode boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/novacode`.
- Do not replace private `packages/novacode` imports with `@novacode-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@novacode-ai/tui`.

## Ownership Boundary

### `@novacode-ai/tui` Owns
...
- NovaCode domain data displayed by the TUI
...
```text
packages/novacode/src/cli/cmd/tui
```

Its private dependency on `packages/novacode` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/novacode/src/*`.
...
Status: Completed. The private `@novacode-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@novacode-ai/tui`.
...
- The package has no dependency on `novacode`, `@novacode-ai/cli`, or
  `@novacode-ai/core`.
...
primitives now live in `@novacode-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@novacode-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads NovaCode globals or process state;
...
Replace process-global NovaCode state with resolved TUI inputs.
...
`@novacode-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/novacode` initially.
...
canonical in `@novacode-ai/tui`. Configured references resolve through the new
...
adapters rather than NovaCode domain implementations.

Make the SDK the only NovaCode domain boundary used by the TUI.
...
- Domain-facing TUI code imports NovaCode data and operations only from
  `@novacode-ai/sdk`.
...
`@novacode-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/novacode`.
...
- The package has no imports from `packages/novacode`, `packages/cli`, or
  `@novacode-ai/core`.
...
public `@novacode-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/novacode`.
- Keep the legacy embedded worker and server startup in `packages/novacode`.
- Change those adapters to load config, create transport inputs, and call the
  public `@novacode-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/novacode`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@novacode-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@novacode-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for NovaCode domain state.
...
cd packages/novacode && bun typecheck
...
rg '@novacode-ai/core|packages/novacode|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/novacode/src packages/novacode/test
```
...
- Build the current-platform `packages/novacode` binary.
