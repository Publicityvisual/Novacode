...
Both `action` and `resource` use novacode's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.novacode` files is intentionally deferred until `.novacode` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether novacode permits an operation through its managed execution paths.
