...
The novacode server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@novacode-ai/llm` tool loop after replacing its
...
- plugins get global "novacode" instance like in that post i showed
- novacode instance has stuff like `novacode.session.prompt()` or
  `novacode.tool.register({...})`
...
- decide whether to preserve deprecated `@novacode-ai/llm` orchestration exports
