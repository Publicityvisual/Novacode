...
Create a small workspace package that vendors the Drizzle `effect-sqlite` adapter shape for our repo. This is not an novacode storage abstraction. It is a local package that ports the Drizzle Effect SQLite implementation so we can use it before/independently of upstream release timing.

`packages/novacode` will use it internally, but the package itself should be generic: Drizzle + Effect + SQLite. No novacode paths, migrations, tables, transaction hooks, post-commit behavior, or domain language should live in this package.
...
- `@novacode-ai/effect-drizzle-sqlite`
...
- Opencode-specific path/channel selection stays in `packages/novacode`.

## Opencode Adoption Notes

These are not package requirements, but they matter for the later novacode adoption PR.

The current `packages/novacode/src/storage/db.ts` has two non-obvious semantics that the novacode wrapper must preserve when it consumes this adapter:
...
The novacode wrapper can implement that using Effect context instead of `LocalContext`:
...
Do not remove this behavior while moving novacode to Effect SQLite. `SyncEvent.run` depends on transaction composability and `behavior: "immediate"` for sequencing correctness.

## Migration Strategy

1. Add `@novacode-ai/effect-drizzle-sqlite` with a minimal in-memory/file SQLite test schema.
...
4. Add `@novacode-ai/effect-drizzle-sqlite` as a dependency of `packages/novacode`.
5. Port `packages/novacode/src/storage/db.ts` to be a thin compatibility wrapper over the adapter plus novacode-specific transaction/post-commit context.
...
8. Only then build domain stores like session/message/project stores on top of novacode's storage wrapper.
...
An Effect Drizzle SQLite package lets us vendor the adapter once. Then novacode can build its own storage wrapper on top, and `SessionStorage`, `MessageStorage`, event store, and projector writes can all share the same transaction and migration model.
...
- Should `afterCommit` stay novacode-specific until event publishing moves? Default answer: yes.
...
- Do CLI/admin raw SQL and sqlite shell stay in `packages/novacode`, or does the storage package expose backend capabilities for them?
...
- Use a tiny test schema, not novacode domain tables.
- Prove Effect Drizzle SQLite queries, transactions, and migrations.
- Do not migrate `packages/novacode` yet except possibly adding the dependency if needed for typechecking.

That gives us a focused place to validate the Effect SQLite approach before disturbing novacode's current database runtime.
