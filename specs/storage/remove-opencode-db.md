# Remove `packages/novacode/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/novacode/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the novacode legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/novacode/src/storage/db.ts` are concentrated in 22 source files:

- `packages/novacode/src/account/repo.ts`
- `packages/novacode/src/cli/cmd/db.ts`
- `packages/novacode/src/cli/cmd/import.ts`
- `packages/novacode/src/cli/cmd/stats.ts`
- `packages/novacode/src/control-plane/workspace.ts`
- `packages/novacode/src/index.ts`
- `packages/novacode/src/node.ts`
- `packages/novacode/src/permission/index.ts`
- `packages/novacode/src/project/project.ts`
- `packages/novacode/src/server/projectors.ts`
- `packages/novacode/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/novacode/src/server/shared/fence.ts`
- `packages/novacode/src/session/message-v2.ts`
- `packages/novacode/src/session/projectors.ts`
- `packages/novacode/src/session/prompt.ts`
- `packages/novacode/src/session/session.ts`
- `packages/novacode/src/session/todo.ts`
- `packages/novacode/src/share/share-next.ts`
- `packages/novacode/src/storage/db.ts`
- `packages/novacode/src/sync/index.ts`
- `packages/novacode/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy novacode database wrapper; `packages/novacode/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the novacode wrapper.
...
Status: Completed. `SyncEvent` and the novacode projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy novacode database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy novacode database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy novacode database wrapper.
...
All migration groups are complete or superseded. `packages/novacode/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No novacode data-migration group remains.

The previous novacode `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate novacode data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/novacode`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/novacode/src`
- `bun typecheck` from `packages/novacode`
- Relevant package tests from `packages/novacode`, not the repo root
