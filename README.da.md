...
# Pakkehåndteringer
npm i -g novacode-ai@latest        # eller bun/pnpm/yarn
scoop install novacode             # Windows
choco install novacode             # Windows
brew install anomalyco/tap/novacode # macOS og Linux (anbefalet, altid up to date)
brew install novacode              # macOS og Linux (officiel brew formula, opdateres sjældnere)
sudo pacman -S novacode            # Arch Linux (Stable)
paru -S novacode-bin               # Arch Linux (Latest from AUR)
mise use -g novacode               # alle OS
nix run nixpkgs#novacode           # eller github:anomalyco/novacode for nyeste dev-branch
```
...
NovaCode findes også som desktop-app. Download direkte fra [releases-siden](https://github.com/anomalyco/novacode/releases) eller [novacode.ai/download](https://novacode.ai/download).
...
NovaCode har to indbyggede agents, som du kan skifte mellem med `Tab`-tasten.
...
Læs mere om [agents](https://novacode.ai/docs/agents).

### Dokumentation

For mere info om konfiguration af NovaCode, [**se vores docs**](https://novacode.ai/docs).

### Bidrag

Hvis du vil bidrage til NovaCode, så læs vores [contributing docs](./CONTRIBUTING.md) før du sender en pull request.

### Bygget på NovaCode

Hvis du arbejder på et projekt der er relateret til NovaCode og bruger "novacode" som en del af navnet; f.eks. "novacode-dashboard" eller "novacode-mobile", så tilføj en note i din README, der tydeliggør at projektet ikke er bygget af NovaCode-teamet og ikke er tilknyttet os på nogen måde.

---

**Bliv en del af vores community** [Discord](https://discord.gg/novacode) | [X.com](https://x.com/novacode)
