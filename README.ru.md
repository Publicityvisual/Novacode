...
# Менеджеры пакетов
npm i -g novacode-ai@latest        # или bun/pnpm/yarn
scoop install novacode             # Windows
choco install novacode             # Windows
brew install anomalyco/tap/novacode # macOS и Linux (рекомендуем, всегда актуально)
brew install novacode              # macOS и Linux (официальная формула brew, обновляется реже)
sudo pacman -S novacode            # Arch Linux (Stable)
paru -S novacode-bin               # Arch Linux (Latest from AUR)
mise use -g novacode               # любая ОС
nix run nixpkgs#novacode           # или github:anomalyco/novacode для самой свежей ветки dev
```
...
NovaCode также доступен как десктопное приложение. Скачайте его со [страницы релизов](https://github.com/anomalyco/novacode/releases) или с [novacode.ai/download](https://novacode.ai/download).
...
4. `$HOME/.novacode/bin` - Fallback по умолчанию
...
В NovaCode есть два встроенных агента, между которыми можно переключаться клавишей `Tab`.
...
Подробнее об [agents](https://novacode.ai/docs/agents).

### Документация

Больше информации о том, как настроить NovaCode: [**наши docs**](https://novacode.ai/docs).

### Вклад

Если вы хотите внести вклад в NovaCode, прочитайте [contributing docs](./CONTRIBUTING.md) перед тем, как отправлять pull request.

### Разработка на базе NovaCode

Если вы делаете проект, связанный с NovaCode, и используете "novacode" как часть имени (например, "novacode-dashboard" или "novacode-mobile"), добавьте примечание в README, чтобы уточнить, что проект не создан командой NovaCode и не аффилирован с нами.

---

**Присоединяйтесь к нашему сообществу** [Discord](https://discord.gg/novacode) | [X.com](https://x.com/novacode)
