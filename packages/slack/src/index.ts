...
import { createOpencode, type ToolPart } from "@novacode-ai/sdk"
...
console.log("🚀 Starting novacode server...")
const novacode = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await novacode.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new novacode session...")
    const { client, server } = novacode
    ...
    console.log("✅ Created novacode session:", createResult.data.id)
  ...
  console.log("📝 Sending to novacode:", message.text)
