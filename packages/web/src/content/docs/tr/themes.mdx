...
novacode ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak novacode kendi `novacode` temasını kullanır.
...
novacode birden fazla yerleşik temayla gelir.
...
- novacode'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
novacode, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/novacode/themes/*.json` veya `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.novacode/themes/*.json`
4. **Geçerli çalışma dizini** - `./.novacode/themes/*.json`
