...
_Model Bağlam Protokolü_ veya MCP kullanarak novacode'a harici araçlar ekleyebilirsiniz. novacode hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [novacode Config](https://novacode.ai/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/novacode` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
novacode, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde novacode şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa novacode, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `novacode mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından novacode, belirteçleri `~/.local/share/novacode/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra novacode'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi novacode yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve novacode'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
