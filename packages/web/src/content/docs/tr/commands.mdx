...
`.novacode/commands/test.md` oluştur:
...
novacode işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

novacode [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/novacode/commands/`
- Proje başına: `.novacode/commands/`
...
novacode, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
