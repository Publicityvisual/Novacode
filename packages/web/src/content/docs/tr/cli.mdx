---
title: CLI
description: novacode CLI seçenekleri ve komutları.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

novacode CLI herhangi bir argüman olmadan çalıştırıldığında varsayılan olarak [TUI](/docs/tui)'yi başlatır.

```bash
novacode
```

Ancak bu sayfada belgelendiği gibi komutları da kabul eder. Bu, novacode ile programlı olarak etkileşim kurmanıza olanak tanır.
...
novacode terminal kullanıcı arayüzünü başlatır.
...
novacode CLI ayrıca aşağıdaki komutlara da sahiptir.

---

### agent

novacode için agent'ları yönetin.
...
`serve` veya `web` komutlarıyla başlatılan, halihazırda çalışan bir novacode arka uç sunucusuna bir terminal ekleyin.

```bash
novacode attach [url]
```

Bu, TUI öğesinin uzak bir novacode arka ucuyla kullanılmasına olanak tanır. Örneğin:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `novacode`) |
...
novacode, [Models.dev](https://models.dev) adresindeki sağlayıcı listesi tarafından desteklenmektedir, dolayısıyla kullanmak istediğiniz herhangi bir sağlayıcı için API anahtarlarını ayarlamak üzere `novacode auth login` kullanabilirsiniz. Bu `~/.local/share/novacode/auth.json`'de saklanıyor.

```bash
novacode auth login
```

novacode başlatıldığında sağlayıcıları kimlik bilgileri dosyasından yükler. Ayrıca ortam değişkenlerindeki veya projenizdeki `.env` dosyasındaki anahtarları da kullanır.
...
Önbelleğe alınan model listesini güncellemek için `--refresh` bayrağını kullanın. Bu, bir sağlayıcıya yeni modeller eklendiğinde ve bunları novacode'da görmek istediğinizde kullanışlıdır.
...
Doğrudan bir istem ileterek novacode'u etkileşimli olmayan modda çalıştırın.
...
Ayrıca, her çalıştırmada MCP sunucusunun soğuk önyükleme sürelerini önlemek için çalışan bir `novacode serve` örneğine de ekleyebilirsiniz:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Çalışan bir novacode sunucusuna ekleyin (ör. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Temel kimlik doğrulama parolası (varsayılan: `OPENCODE_SERVER_PASSWORD`)                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Temel kimlik doğrulama kullanıcı adı (varsayılan: `OPENCODE_SERVER_USERNAME` veya `novacode`) |
...
API erişimi için headless bir novacode sunucusu başlatır. Tam HTTP arayüzü için [server docs](/docs/server) sayfasına bakın.

```bash
novacode serve
```

Bu, TUI arayüzü olmadan novacode işlevselliğine API erişimi sağlayan bir HTTP sunucusunu başlatır. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `novacode` şeklindedir).
...
Tüm novacode oturumlarını listeleyin.
...
novacode oturumlarınız için belirteç kullanımı ve maliyet istatistiklerini gösterin.
...
Bir JSON dosyasından veya novacode paylaşımından URL oturum verilerini içe aktarın.

```bash
novacode import <file>
```

Yerel bir dosyadan veya novacode paylaşımından (URL) içe aktarabilirsiniz.
...
Web arayüzüyle başsız bir novacode sunucusu başlatın.

```bash
novacode web
```

Bu, bir HTTP sunucusunu başlatır ve bir web arayüzü aracılığıyla novacode'a erişmek için bir web tarayıcısı açar. HTTP temel kimlik doğrulamasını etkinleştirmek için `OPENCODE_SERVER_PASSWORD` öğesini ayarlayın (kullanıcı adı varsayılan olarak `novacode` şeklindedir).
...
novacode'u kaldırın ve ilgili tüm dosyaları kaldırın.
...
novacode'u en son sürüme veya belirli bir sürüme günceller.
...
novacode CLI aşağıdaki global bayrakları destekler.
...
novacode ortam değişkenleri kullanılarak yapılandırılabilir.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Temel kimlik doğrulama kullanıcı adını geçersiz kıl (varsayılan `novacode`) |
