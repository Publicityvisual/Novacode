...
novacode ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\novacode\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `novacode --log-level DEBUG`.

---

## Depolama

novacode, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:
...
novacode Desktop arka planda yerel bir novacode sunucusu (`novacode-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `novacode` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (veya `~/.config/novacode/novacode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/novacode/novacode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\novacode\novacode.jsonc` yapistirin
...
novacode diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/novacode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\novacode\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), novacode'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. novacode Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/novacode`
- **Linux**: `~/.cache/novacode` dizinini silin (veya `rm -rf ~/.cache/novacode` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\novacode` yapistirin

3. novacode Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

novacode Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`novacode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
...
Windows'ta novacode Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, novacode ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

novacode Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda novacode icin bildirimler etkinse
...
1. novacode Desktop'u kapatin
2. Su dosyalari bulun ve silin (novacode Desktop uygulama veri dizininde yer alirlar):

- `novacode.settings.dat` (desktop varsayilan sunucu URL'si)
- `novacode.global.dat` ve `novacode.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
novacode ile ilgili bir sorun yasiyorsaniz:
...
3. `novacode upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `novacode models` calistirin.
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. novacode, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
2. En guncel provider paketlerini yeniden kurmak icin novacode'u yeniden baslatin

Bu, novacode'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
novacode, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
