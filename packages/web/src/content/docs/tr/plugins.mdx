---
title: Eklentiler
description: NovaCode'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek NovaCode'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya NovaCode'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.novacode/plugins/` - Proje düzeyinde eklentiler
- `~/.config/novacode/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/novacode/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/novacode/novacode.json`)
2. Project config (`novacode.json`)
3. Global eklenti dizini (`~/.config/novacode/plugins/`)
4. Proje eklenti dizini (`.novacode/plugins/`)
...
NovaCode bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir NovaCode SDK istemcisi.
...
NovaCode'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
NovaCode masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
NovaCode'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca NovaCode'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, NovaCode'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra NovaCode'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://novacode.ai/docs/sdk) bakın.
