...
Ajan becerileri, novacode'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
novacode su konumlari tarar:

- Proje konfigurasyonu: `.novacode/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/novacode/skills/<name>/SKILL.md`
...
Proje yerel yollarinda novacode, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.novacode/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.novacode/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
novacode, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`novacode.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`novacode.json` icinde):
