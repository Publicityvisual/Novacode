---
title: CLI
description: NovaCode CLI options and commands.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

The NovaCode CLI by default starts the [TUI](/docs/tui) when run without any arguments.

```bash
novacode
```

But it also accepts commands as documented on this page. This allows you to interact with NovaCode programmatically.
...
Start the NovaCode terminal user interface.
...
The NovaCode CLI also has the following commands.

---

### agent

Manage agents for NovaCode.
...
| Flag                                        | Short | Description                                                                                                                                                                                                                |
| ------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| <nobr><code>{"--path"}</code></nobr>        |       | Directory to write the agent file to (defaults to global or `.novacode/agent` based on the prompt)                                                                                                                         |
...
Attach a terminal to an already running NovaCode backend server started via `serve` or `web` commands.

```bash
novacode attach [url]
```

This allows using the TUI with a remote NovaCode backend. For example:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `novacode`) |
...
NovaCode is powered by the provider list at [Models.dev](https://models.dev), so you can use `novacode auth login` to configure API keys for any provider you'd like to use. This is stored in `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

When NovaCode starts up it loads the providers from the credentials file. And if there are any keys defined in your environments or a `.env` file in your project.
...
Use the `--refresh` flag to update the cached model list. This is useful when new models have been added to a provider and you want to see them in NovaCode.
...
Run novacode in non-interactive mode by passing a prompt directly.
...
You can also attach to a running `novacode serve` instance to avoid MCP server cold boot times on every run:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach to a running novacode server (e.g., http://localhost:4096)          |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Basic auth password (defaults to `OPENCODE_SERVER_PASSWORD`)               |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Basic auth username (defaults to `OPENCODE_SERVER_USERNAME` or `novacode`) |
...
Start a headless NovaCode server for API access. Check out the [server docs](/docs/server) for the full HTTP interface.

```bash
novacode serve
```

This starts an HTTP server that provides API access to novacode functionality without the TUI interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `novacode`).
...
Delete an NovaCode session.

```bash
novacode session delete <sessionID>
```
...
Show token usage and cost statistics for your NovaCode sessions.
...
Import session data from a JSON file or NovaCode share URL.

```bash
novacode import <file>
```

You can import from a local file or an NovaCode share URL.
...
Start a headless NovaCode server with a web interface.

```bash
novacode web
```

This starts an HTTP server and opens a web browser to access NovaCode through a web interface. Set `OPENCODE_SERVER_PASSWORD` to enable HTTP basic auth (username defaults to `novacode`).
...
```bash
novacode plugin <module>
```

Or use the alias.

```bash
novacode plug <module>
```
...
Fetch and checkout a GitHub PR branch, then run NovaCode.
...
Uninstall NovaCode and remove all related files.
...
Updates novacode to the latest version or a specific version.
...
The novacode CLI takes the following global flags.
...
NovaCode can be configured using environment variables.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Override basic auth username (default `novacode`) |
