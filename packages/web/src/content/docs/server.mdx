---
title: Server
description: Interact with novacode server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `novacode serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an novacode client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `novacode.local` |
...
Set `OPENCODE_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `novacode`, or set `OPENCODE_SERVER_USERNAME` to override it. This applies to both `novacode serve` and `novacode web`.
...
When you run `novacode` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the novacode server to interact with novacode programmatically.
:::

This architecture lets novacode support multiple clients and allows you to interact with novacode programmatically.

You can run `novacode serve` to start a standalone server. If you have the
novacode TUI running, `novacode serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the NovaCode [IDE](/docs/ide) plugins.
...
The novacode server exposes the following APIs.
