...
Tools allow the LLM to perform actions in your codebase. NovaCode comes with a set of built-in tools, but you can extend it with [custom tools](/docs/custom-tools) or [MCP servers](/docs/mcp-servers).
...
Here are all the built-in tools available in NovaCode.
...
:::note
This tool is only available when using the NovaCode or NovaCode Go provider, or when either the `OPENCODE_ENABLE_EXA` or `OPENCODE_ENABLE_PARALLEL` environment variable is set to any truthy value (e.g., `true` or `1`).

To enable when launching NovaCode:

```bash
OPENCODE_ENABLE_EXA=1 novacode
# or
OPENCODE_ENABLE_PARALLEL=1 novacode
```
