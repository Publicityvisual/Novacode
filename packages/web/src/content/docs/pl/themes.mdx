...
Dzięki novacode możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie novacode używa naszego własnego motywu `novacode`.
...
novacode ma kilka wbudowanych motywów.
...
- Chcą, aby NovaCode pasował do wyglądu ich terminala
...
novacode obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/novacode/themes/*.json` lub `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.novacode/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.novacode/themes/*.json`
