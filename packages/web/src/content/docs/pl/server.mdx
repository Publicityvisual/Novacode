---
title: Serwer
description: Interakcja z serwerem novacode za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `novacode serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient novacode.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `novacode.local` |
...
Ustaw `OPENCODE_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `novacode` lub ustaw `OPENCODE_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `novacode serve`, jak i `novacode web`.
...
Kiedy uruchomisz `novacode`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera novacode do programowej interakcji z novacode.
:::

Ta architektura umożliwia obsługę wielu klientów przez novacode i programową interakcję z kodem otwartym.

Możesz uruchomić `novacode serve`, aby uruchomić samodzielny serwer. Jeśli masz
novacode TUI działa, `novacode serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki novacode [IDE](/docs/ide).
...
Serwer novacode udostępnia następujące interfejsy API.
