...
Aby debugować problemy z novacode, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `novacode --log-level DEBUG`.

---

## Przechowywanie danych

novacode przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:
...
novacode Desktop uruchamia w tle lokalny serwer novacode (sidecar `novacode-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `NovaCode` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (lub `~/.config/novacode/novacode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/novacode/novacode.jsonc`
...
novacode może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby novacode mógł ją odbudować.

1. Całkowicie zamknij novacode Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/novacode`
- **Linux**: usuń `~/.cache/novacode` (lub uruchom `rm -rf ~/.cache/novacode`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\novacode`

3. Uruchom ponownie novacode Desktop.

---

### Napraw problemy z połączeniem z serwerem

novacode Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `novacode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
...
W systemie Windows novacode Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami novacode.

---

### Brak powiadomień

novacode Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla novacode w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie novacode Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji novacode Desktop):

- `novacode.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `novacode.global.dat` i `novacode.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z novacode:
...
3. Upewnij się, że masz najnowszą wersję z `novacode upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `novacode models`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. novacode dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
2. Uruchom ponownie kod novacode, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to novacode do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
novacode wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
