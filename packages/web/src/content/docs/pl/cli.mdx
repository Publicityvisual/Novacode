---
title: CLI
description: Opcje i polecenia wiersza poleceń NovaCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CLI NovaCode domyślnie uruchamia [TUI](/docs/tui), gdy wywołasz je bez argumentów.

```bash
novacode
```

CLI obsługuje też polecenia opisane na tej stronie, więc możesz używać NovaCode programowo.
...
Uruchamia interfejs terminalowy (TUI) NovaCode.
...
Interfejs CLI NovaCode zawiera także dodatkowe polecenia.

---

### agent

Zarządzaj agentami NovaCode.
...
Dołącz terminal do już działającego serwera backendu NovaCode uruchomionego za pomocą `serve` lub `web`.

```bash
novacode attach [url]
```

Pozwala to połączyć TUI ze zdalnym backendem NovaCode. Na przykład:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `novacode`) |
...
NovaCode integruje się z wieloma dostawcami. Możesz użyć `novacode auth login`, aby skonfigurować klucz API dla dowolnego obsługiwanego dostawcy. Klucze są bezpiecznie przechowywane w `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Podczas uruchamiania NovaCode ładuje dostawców z pliku poświadczeń, a także ze zmiennych środowiskowych lub pliku `.env` w Twoim projekcie.
...
Użyj flagi `--refresh`, aby zaktualizować listę modeli w pamięci podręcznej. Jest to przydatne, gdy dostawca dodał nowe modele, które chcesz zobaczyć w NovaCode.
...
Uruchom NovaCode w trybie nieinteraktywnym, przekazując bezpośrednio prompt.
...
Można również użyć tego z działającym `novacode serve`, aby uniknąć zimnego startu serwera MCP przy każdym uruchomieniu:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Dołącz do działającego serwera NovaCode (np. http://localhost:4096)                                   |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Hasło uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_PASSWORD`)                            |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nazwa użytkownika uwierzytelniania podstawowego (domyślnie `OPENCODE_SERVER_USERNAME` lub `novacode`) |
...
Uruchom serwer NovaCode (bez interfejsu) w celu uzyskania dostępu do API. Pełny opis API HTTP znajduje się w [dokumentacji serwera](/docs/server).

```bash
novacode serve
```

Uruchamia to serwer HTTP, który zapewnia dostęp do API NovaCode bez interfejsu TUI. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `novacode`).
...
Lista wszystkich sesji NovaCode.
...
Pokaż statystyki wykorzystania tokenów i sesji NovaCode.
...
Importuj dane sesji z pliku JSON lub adresu URL udostępniania NovaCode.
...
Uruchom serwer NovaCode z interfejsem internetowym.

```bash
novacode web
```

Uruchamia to serwer HTTP i udostępnia NovaCode przez interfejs przeglądarkowy. Ustaw `OPENCODE_SERVER_PASSWORD`, aby włączyć podstawowe uwierzytelnianie HTTP (domyślna nazwa użytkownika to `novacode`).
...
Odinstaluj NovaCode i usuń wszystkie powiązane pliki.
...
Aktualizuj NovaCode do najnowszej lub określonej wersji.
...
Interfejs CLI NovaCode przyjmuje następujące flagi globalne dla każdego polecenia.
...
NovaCode można skonfigurować za pomocą zmiennych środowiskowych.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Nazwa użytkownika do autoryzacji (domyślnie `novacode`)    |
