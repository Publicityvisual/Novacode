---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w novacode.
---

import config from "../../../../config.mjs"
export const console = config.console

novacode używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji novacode.
...
w `~/.local/share/novacode/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim novacode
config.
...
NovaCode Zen to lista modeli dostarczonych przez zespół novacode, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z novacode. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od NovaCode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `NovaCode Zen` i przejdź do [novacode.ai/auth](https://novacode.ai/zen).
...
Działa jak każdy inny dostawca w novacode i jest całkowicie opcjonalny w użyciu.

---

## NovaCode Go

NovaCode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół novacode, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z novacode.

1. Uruchom polecenie `/connect` w TUI, wybierz `NovaCode Go` i przejdź do [novacode.ai/auth](https://novacode.ai/zen).
...
Aby używać Amazon Bedrock z novacode:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania novacode:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `novacode.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w novacode nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować novacode do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod novacode działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji novacode.
...
novacode używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować novacode
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`novacode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem novacode:
...
Aby używać Google Vertex AI z novacode:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania novacode.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą novacode, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session), aby automatycznie rejestrować każdą konwersację novacode jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja novacode będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować novacode tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować novacode tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować novacode tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem novacode. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/novacode).
:::
...
Aby korzystać z Ollama Cloud z novacode:
...
3. Skopiuj klucz API do użycia w novacode.
...
6. **Ważne**: Przed użyciem modeli chmurowych w novacode musisz pobrać informacje o modelu lokalnie:
...
NovaCode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół novacode. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>NovaCode Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **NovaCode Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API novacode.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji novacode.
...
5. Możesz je także dostosować za pomocą konfiguracji novacode. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z novacode:
...
Możesz także dostosować modele za pomocą konfiguracji novacode. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z novacode. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `novacode.json` w katalogu projektu:
...
Pola `limit` pozwalają novacode zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `novacode auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację novacode i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji novacode.
