...
Możesz dodać dodatkowe informacje do novacode za pomocą _Model Context Protocol_, w skrócie MCP. novacode obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [novacode Config](https://novacode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/novacode`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
novacode automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, novacode:
...
Jeśli serwer wymaga uwierzytelnienia, novacode poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `novacode mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji novacode bezpieczne przechowa tokeny w `~/.local/share/novacode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w novacode, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie novacode, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć novacode z kontem Sentry.
