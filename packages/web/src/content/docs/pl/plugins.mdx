---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć novacode.
---

Wtyczki umożliwiające wyłączenie novacode poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania novacode.
...
- `.novacode/plugins/` - Wtyczki na poziomie projektu
- `~/.config/novacode/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/novacode/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/novacode/novacode.json`)
2. Konfiguracja projektu (`novacode.json`)
3. Globalny katalog wtyczek (`~/.config/novacode/plugins/`)
4. Katalog wtyczek projektu (`.novacode/plugins/`)
...
novacode uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z novacode do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do novacode.
...
:::note
Jeśli korzystasz z aplikacji komputerowej novacode, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj novacode czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do novacode:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod novacode. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://novacode.ai/docs/sdk).
