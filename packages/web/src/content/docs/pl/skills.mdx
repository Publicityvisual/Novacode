...
Umiejętności agenta pozwalają novacode odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
novacode przeszukuje te lokalizacje:

- Project config: `.novacode/skills/<name>/SKILL.md`
- Global config: `~/.config/novacode/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu novacode przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.novacode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.novacode/skills/git-release/SKILL.md` w ten sposób:
...
novacode wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `novacode.json`:
...
**Dla agentów wbudowanych** (w `novacode.json`):
