...
NovaCode Go to niskokosztowa subskrypcja za **10 $/miesiąc**, która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w NovaCode. Subskrybujesz NovaCode Go i
...
NovaCode Go daje Ci dostęp do tych modeli za **10 $/miesiąc**.

---

## Jak to działa

NovaCode Go działa jak każdy inny dostawca w NovaCode.

1. Logujesz się do **<a href={console}>NovaCode Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `NovaCode Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować NovaCode Go.
:::
...
NovaCode Go jest przeznaczony dla [NovaCode](https://novacode.ai) i innych agentów kodujących,
...
3. Wysyłać stabilny identyfikator sesji w nagłówku `x-novacode-session` dla każdej rozmowy, aby umożliwić optymalizację routingu i
   buforowania promptów.

### Zweryfikowane klienty

Poza NovaCode zweryfikowano, że poniżsi klienci działają prawidłowo
z NovaCode Go. Nie gwarantujemy jednak, że będą nadal działać w przyszłości.

| Klient            | Obsługa sesji                                                                                                                                                                                                                                              |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Kompilacje zawierające [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) wysyłają nagłówek w głównych i pomocniczych żądaniach NovaCode. Poprawkę scalono po wersji v0.21.0; samo to wydanie jej nie zawiera.                         |
...
| **ZCode**         | Go rozpoznaje jego natywny nagłówek sesji. Nasza [prośba o `x-novacode-session`](https://github.com/zai-org/feedback/issues/492) pozostaje otwarta, ale wysyłanie tego konkretnego nagłówka nie jest już konieczne.                                        |
| **Pi**            | Aktualne kompilacje wysyłają informacje o sesji dla NovaCode. Zaktualizuj starsze instalacje.                                                                                                                                                              |
...
| **Kilo Code CLI** | Kompilacje zawierające [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) przywracają nagłówki sesji NovaCode. Ta poprawka obejmuje CLI, ale nie rozszerzenie VS Code. Zobacz [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji NovaCode
używa formatu `novacode-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`novacode-go/kimi-k3` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z NovaCode.
