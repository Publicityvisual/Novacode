...
Pour déboguer les problèmes avec NovaCode, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `novacode --log-level DEBUG`.

---

## Stockage

novacode stocke les données de session et autres données d'application sur le disque à l'emplacement :
...
NovaCode Desktop exécute un serveur NovaCode local (le side-car `novacode-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `NovaCode` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/novacode/novacode.jsonc`
...
NovaCode peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que NovaCode puisse le reconstruire.

1. Quittez complètement NovaCode Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/novacode`
- **Linux** : supprimez `~/.cache/novacode` (ou exécutez `rm -rf ~/.cache/novacode`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\novacode`

3. Redémarrez le bureau NovaCode.

---

### Résoudre les problèmes de connexion au serveur

NovaCode Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `novacode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
...
Sur Windows, NovaCode Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de NovaCode.

---

### Les notifications ne s'affichent pas

NovaCode Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour NovaCode dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau NovaCode.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application NovaCode Desktop) :

- `novacode.settings.dat` (serveur de bureau par défaut URL)
- `novacode.global.dat` et `novacode.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec NovaCode :
...
3. Assurez-vous d'avoir la dernière version avec `novacode upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `novacode models`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. novacode installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
2. Redémarrez novacode pour réinstaller les derniers packages du fournisseur

Cela forcera novacode à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
novacode détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
