---
title: Plugins
description: Écrivez vos propres plugins pour étendre NovaCode.
---

Les plugins vous permettent d'étendre NovaCode en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de NovaCode.
...
- `.novacode/plugins/` - Plugins au niveau du projet
- `~/.config/novacode/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/novacode/node_modules/`.
...
1. Configuration globale (`~/.config/novacode/novacode.json`)
2. Configuration du projet (`novacode.json`)
3. Répertoire global des plugins (`~/.config/novacode/plugins/`)
4. Répertoire des plugins du projet (`.novacode/plugins/`)
...
NovaCode exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK novacode pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre novacode.
...
:::note
Si vous utilisez l'application de bureau NovaCode, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez novacode de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à novacode :
...
L'assistant `tool` crée un outil personnalisé que novacode peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour novacode aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://novacode.ai/docs/sdk) pour plus de détails.
