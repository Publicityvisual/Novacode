---
title: Support ACP
description: Utilisez NovaCode dans n’importe quel éditeur compatible ACP.
---

NovaCode prend en charge le [Agent Client Protocol](https://agentclientprotocol.com) (ACP), vous permettant de l'utiliser directement dans les éditeurs et IDE compatibles.
...
Pour utiliser NovaCode via ACP, configurez votre éditeur pour exécuter la commande `novacode acp`.

La commande démarre NovaCode en tant que sous-processus compatible ACP qui communique avec votre éditeur via JSON-RPC sur stdio.
...
Installez NovaCode depuis le [registre ACP de Zed](https://zed.dev/docs/ai/external-agents#registry) en exécutant `zed: acp registry` dans la palette de commandes.

Pour utiliser plutôt un exécutable NovaCode personnalisé, ajoutez-le à votre configuration [Zed](https://zed.dev) (`~/.config/zed/settings.json`) :
...
Pour l'ouvrir, utilisez le nouvel agent « NovaCode » dans le sélecteur d'agent AI Chat.
...
Pour utiliser NovaCode comme agent ACP dans [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), ajoutez ce qui suit à votre configuration Neovim :
...
Cette configuration configure CodeCompanion pour utiliser NovaCode comme agent ACP pour le chat.
...
NovaCode fonctionne de la même manière via ACP que dans le terminal. Toutes les fonctionnalités sont prises en charge :
...
- Serveurs MCP configurés dans votre configuration NovaCode
