...
Avec NovaCode, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, NovaCode utilise notre propre thème `novacode`.
...
NovaCode est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que NovaCode corresponde à l'apparence de leur terminal
...
NovaCode prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/novacode/themes/*.json` ou `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.novacode/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.novacode/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/novacode/themes
vim ~/.config/novacode/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .novacode/themes
vim .novacode/themes/my-theme.json
```
...
{
  "$schema": "https://novacode.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
