---
title: Plugins
description: Skriv dine egne plugins for at udvide NovaCode.
---

Plugins giver dig mulighed for at udvide NovaCode ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre NovaCodes standardadfærd.
...
- `.novacode/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/novacode/node_modules/`.
...
1. Global konfiguration (`~/.config/novacode/novacode.json`)
2. Projektkonfiguration (`novacode.json`)
3. Global plugin-mappe (`~/.config/novacode/plugins/`)
4. Projekt plugin bibliotek (`.novacode/plugins/`)
...
NovaCode kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En novacode SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide novacode.
...
:::note
Hvis du bruger NovaCode desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå novacode i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til novacode:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som novacode kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for novacode sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://novacode.ai/docs/sdk) for detaljer.
