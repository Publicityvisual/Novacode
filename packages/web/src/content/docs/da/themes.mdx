...
Med NovaCode kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger NovaCode vårt eget `novacode`-tema.
...
NovaCode kommer med flere innebygde temaer.
...
- Vil at NovaCode skal matche terminalens utseende
...
NovaCode støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/novacode/themes/*.json` eller `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.novacode/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.novacode/themes/*.json`
