---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i NovaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

NovaCode bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i NovaCode-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/novacode/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i NovaCode-konfigurationen.
...
NovaCode Zen er en liste over modeller leveret af NovaCode-teamet, som er blevet testet og verificeret til at fungere godt med NovaCode. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med NovaCode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg novacode og gå til [novacode.ai/auth](https://novacode.ai/auth).
...
Det fungerer som alle andre udbydere i NovaCode og er helt valgfrit at bruge.

---

## NovaCode Go

NovaCode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af NovaCode-teamet, som er testet og verificeret til at fungere godt med NovaCode.

1. Kør kommandoen `/connect` i TUI, vælg `NovaCode Go`, og gå til [novacode.ai/auth](https://novacode.ai/zen).
...
Sådan bruger du Amazon Bedrock med NovaCode:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører novacode:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `novacode.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i NovaCode understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere novacode til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at novacode skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem novacode-konfigurationen.
...
gemme tokenet i novacode auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
NovaCode bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse NovaCode
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`novacode.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `novacode.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med NovaCode:
...
Sådan bruger du Google Vertex AI med NovaCode:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører novacode.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem novacode, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session) plugin for automatisk at logge hver NovaCode-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver NovaCode-samtale opført som en separat session.
...
Du kan konfigurere novacode til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere novacode til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere novacode til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for NovaCode. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/novacode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med NovaCode:
...
3. Kopier API-nøglen til brug i NovaCode.
...
6. **Vigtigt**: Før du bruger skymodeller i NovaCode, skal du hente modelinformationen lokalt:
...
NovaCode Zen er en liste over testede og verificerede modeller leveret af NovaCode-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>NovaCode Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **NovaCode Zen**.

   ```txt
   /connect
   ```

3. Indtast NovaCode API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem novacode-konfigurationen.
...
5. Du kan også tilpasse dem gennem novacode-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NovaCode:
...
Du kan også tilpasse modeller gennem novacode-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med novacode. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `novacode.json`-filen i projektkataloget:
...
`limit`-felterne lader NovaCode forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `novacode auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek novacode-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i novacode-konfigurationen.
