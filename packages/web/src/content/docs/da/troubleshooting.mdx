...
For at fejlfinde problemer med NovaCode, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `novacode --log-level DEBUG`.

---

## Lagring

novacode gemmer sessionsdata og andre applikationsdata på disken på:
...
NovaCode Desktop kører en lokal NovaCode-server (`novacode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `NovaCode`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux** (ældre installationer): `~/.local/share/novacode/novacode.jsonc`
...
NovaCode kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så NovaCode kan genopbygge den.

1. Afslut NovaCode Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/novacode`
- **Linux**: slet `~/.cache/novacode` (eller kør `rm -rf ~/.cache/novacode`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\novacode`

3. Start NovaCode Desktop på ny.

---

### Løs problemer med serverforbindelse

NovaCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `novacode.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.
...
På Windows kræver NovaCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i NovaCode.

---

### Meddelelser vises ikke

NovaCode Desktop viser kun systemvarsler når:

- varsler er aktiveret for NovaCode i dine OS-indstillinger, og
...
1. Afslut NovaCode Desktop.
2. Find og slet disse filer (de findes i NovaCode Desktop-appens datamappe):

- `novacode.settings.dat` (skrivebordsstandardserver URL)
- `novacode.global.dat` og `novacode.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med NovaCode:
...
For at finde ud af hvilke modeller du har adgang til, kør `novacode models`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. novacode installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
2. Start novacode på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge novacode til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
novacode vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
