---
title: CLI
description: NovaCode CLI muligheder og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NovaCode CLI starter som standard [TUI](/docs/tui), når den kører uden nogen argumenter.

```bash
novacode
```

Men den accepterer også kommandoer som dokumenteret på denne side. Dette giver dig mulighed for at interagere med NovaCode programmatisk.
...
Start NovaCode terminalbrugergrænsefladen.
...
NovaCode CLI har også følgende kommandoer.

---

### agent

Administrer agenter i NovaCode.
...
Tilslut en terminal til en allerede kørende NovaCode backend-server startet via `serve` eller `web` kommandoer.

```bash
novacode attach [url]
```

Dette gør det muligt at bruge TUI med en ekstern NovaCode backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `novacode`) |
...
NovaCode drives af udbyderlisten på [Models.dev](https://models.dev), så du kan bruge `novacode auth login` til at konfigurere API-nøgler for enhver udbyder, du vil bruge. Dette er gemt i `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Når NovaCode starter op, indlæser den udbyderne fra legitimationsfilen. Og hvis der er nogen nøgler defineret i dine miljøer eller en `.env` fil i dit projekt.
...
Brug flaget `--refresh` til at opdatere den cachelagrede modelliste. Dette er nyttigt, når nye modeller er blevet tilføjet til en udbyder, og du vil se dem i NovaCode.
...
Kør novacode i ikke-interaktiv tilstand ved at sende en prompt direkte.
...
Du kan også vedhæfte til en kørende `novacode serve`-instans for at undgå MCP serverens kolde opstartstider ved hver kørsel:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Tilslut til en kørende novacode-server (f.eks. http://localhost:4096)               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Adgangskode til basic auth (standard: `OPENCODE_SERVER_PASSWORD`)                   |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brugernavn til basic auth (standard: `OPENCODE_SERVER_USERNAME` eller `novacode`)   |
...
Start en hovedløs NovaCode-server til API-adgang. Tjek [server docs](/docs/server) for den fulde HTTP-grænseflade.

```bash
novacode serve
```

Dette starter en HTTP-server, der giver API-adgang til novacode-funktionalitet uden TUI-grænsefladen. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `novacode`).
...
Administrator NovaCode sessionsholder.
...
Liste over alle NovaCode sessioner.
...
Vis tokenbrug og omkostningsstatistikker for dine NovaCode-sessioner.
...
Importer sessionsdata fra en JSON fil eller NovaCode del URL.

```bash
novacode import <file>
```

Du kan importere fra en lokal fil eller en NovaCode share URL.
...
Start en hovedløs NovaCode-server med en webgrænseflade.

```bash
novacode web
```

Dette starter en HTTP-server og åbner en webbrowser for at få adgang til NovaCode via en webgrænseflade. Indstil `OPENCODE_SERVER_PASSWORD` for at aktivere HTTP grundlæggende godkendelse (brugernavn er standard til `novacode`).
...
Afinstaller NovaCode og fjern alle relaterede filer.
...
Opdaterer NovaCode til den nyeste version eller en specifik version.
...
novacode CLI tager følgende globale flag.
...
NovaCode kan konfigureres ved hjælp af miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Tilsidesæt grundlæggende godkendelsesbrugernavn (standard `novacode`) |
