---
title: Server
description: Interager med novacode-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `novacode serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en novacode-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `novacode.local` |
...
Indstil `OPENCODE_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `novacode`, eller indstil `OPENCODE_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `novacode serve` og `novacode web`.
...
Når du kører `novacode` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug novacode-serveren til at interagere med novacode programmatisk.
:::

Denne arkitektur lader novacode understøtte flere klienter og lader dig interagere med novacode programmatisk.

Du kan køre `novacode serve` for at starte en selvstændig server. Hvis du har
novacode TUI kørende, vil `novacode serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af NovaCode [IDE](/docs/ide) plugins.
...
NovaCode-serveren viser følgende API'er.
