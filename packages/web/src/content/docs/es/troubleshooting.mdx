...
Para depurar problemas con NovaCode, comience verificando los registros y los datos locales que almacena en el disco.
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `novacode --log-level DEBUG`.

---

## Almacenamiento

novacode almacena datos de sesión y otros datos de aplicaciones en el disco en:
...
NovaCode Desktop ejecuta un servidor NovaCode local (el sidecar `novacode-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `NovaCode` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (o `~/.config/novacode/novacode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/novacode/novacode.jsonc`
...
NovaCode también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que NovaCode pueda reconstruirlo.

1. Salga de NovaCode Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/novacode`
- **Linux**: eliminar `~/.cache/novacode` (o ejecutar `rm -rf ~/.cache/novacode`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\novacode`

3. Reinicie el escritorio NovaCode.

---

### Solucionar problemas de conexión del servidor

NovaCode Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `novacode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
...
En Windows, el escritorio NovaCode requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de NovaCode.

---

### Notificaciones que no se muestran

NovaCode Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para NovaCode en la configuración de su sistema operativo, y
...
1. Salga del escritorio NovaCode.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio NovaCode):

- `novacode.settings.dat` (URL del servidor predeterminado de escritorio)
- `novacode.global.dat` y `novacode.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con NovaCode:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/novacode/issues**](https://github.com/anomalyco/novacode/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**novacode.ai/discord**](https://novacode.ai/discord)
...
3. Asegúrese de tener la última versión con `novacode upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `novacode models`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. novacode instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
2. Reinicie novacode para reinstalar los paquetes del proveedor más recientes.

Esto obligará a novacode a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
novacode detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
