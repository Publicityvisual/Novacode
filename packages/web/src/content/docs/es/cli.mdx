---
title: CLI
description: Opciones y comandos de la CLI de NovaCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI de NovaCode de forma predeterminada inicia el [TUI](/docs/tui) cuando se ejecuta sin ningún argumento.

```bash
novacode
```

Pero también acepta comandos como se documenta en esta página. Esto le permite interactuar con NovaCode mediante programación.
...
Inicie la interfaz de usuario del terminal NovaCode.
...
La CLI de NovaCode también tiene los siguientes comandos.

---

### agent

Administrar agentes para NovaCode.
...
Conecte una terminal a un servidor backend NovaCode que ya se esté ejecutando y iniciado mediante los comandos `serve` o `web`.

```bash
novacode attach [url]
```

Esto permite usar TUI con un backend remoto NovaCode. Por ejemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `novacode`) |
...
NovaCode funciona con la lista de proveedores en [Models.dev](https://models.dev), por lo que puede usar `novacode auth login` para configurar las claves API para cualquier proveedor que desee utilizar. Esto se almacena en `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Cuando se inicia NovaCode, carga los proveedores desde el archivo de credenciales. Y si hay claves definidas en sus entornos o un archivo `.env` en su proyecto.
...
Utilice el indicador `--refresh` para actualizar la lista de modelos almacenados en caché. Esto es útil cuando se han agregado nuevos modelos a un proveedor y desea verlos en NovaCode.
...
Ejecute novacode en modo no interactivo pasando un mensaje directamente.
...
También puede conectarse a una instancia `novacode serve` en ejecución para evitar tiempos de arranque en frío del servidor MCP en cada ejecución:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Adjuntar a un servidor novacode en ejecución (por ejemplo, http://localhost:4096)         |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Contraseña de autenticación básica (predeterminada: `OPENCODE_SERVER_PASSWORD`)           |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuario de autenticación básica (predeterminado: `OPENCODE_SERVER_USERNAME` u `novacode`) |
...
Inicie un servidor NovaCode sin cabeza para acceso API. Consulte los [documentos del servidor](/docs/server) para conocer la interfaz HTTP completa.

```bash
novacode serve
```

Esto inicia un servidor HTTP que proporciona acceso API a la funcionalidad novacode sin la interfaz TUI. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `novacode`).
...
Enumere todas las sesiones NovaCode.
...
Muestre el uso de tokens y las estadísticas de costos para sus sesiones NovaCode.
...
Importe datos de sesión desde un archivo JSON o una URL compartida NovaCode.

```bash
novacode import <file>
```

Puede importar desde un archivo local o una URL compartida NovaCode.
...
Inicie un servidor NovaCode sin cabeza con una interfaz web.

```bash
novacode web
```

Esto inicia un servidor HTTP y abre un navegador web para acceder a NovaCode a través de una interfaz web. Configure `OPENCODE_SERVER_PASSWORD` para habilitar la autenticación básica HTTP (el nombre de usuario predeterminado es `novacode`).
...
Desinstale NovaCode y elimine todos los archivos relacionados.
...
Actualiza novacode a la última versión o a una versión específica.
...
La CLI de NovaCode toma las siguientes banderas globales.
...
NovaCode se puede configurar mediante variables de entorno.
...
| `OPENCODE_SERVER_USERNAME`            | cadena   | Anular el nombre de usuario de autenticación básica (predeterminado `novacode`) |
