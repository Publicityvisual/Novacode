...
Las habilidades del agente permiten a NovaCode descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
NovaCode busca estas ubicaciones:

- Configuración del proyecto: `.novacode/skills/<name>/SKILL.md`
- Configuración global: `~/.config/novacode/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, NovaCode sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.novacode/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.novacode/skills/git-release/SKILL.md` así:
...
NovaCode enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `novacode.json`:
...
**Para agentes integrados** (en `novacode.json`):
