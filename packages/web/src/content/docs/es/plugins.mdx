---
title: Complementos
description: Escriba sus propios complementos para extender NovaCode.
---

Los complementos le permiten extender NovaCode al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de NovaCode.
...
- `.novacode/plugins/` - Complementos a nivel de proyecto
- `~/.config/novacode/plugins/` - Complementos globales
...
```json title="novacode.json"
{
  ...
  "plugin": ["novacode-helicone-session", "novacode-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/novacode/node_modules/`.
...
1. Configuración global (`~/.config/novacode/novacode.json`)
2. Configuración del proyecto (`novacode.json`)
3. Directorio global de complementos (`~/.config/novacode/plugins/`)
4. Directorio de complementos del proyecto (`.novacode/plugins/`)
...
```json title=".novacode/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
NovaCode ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".novacode/plugins/my-plugin.ts"
...
```js title=".novacode/plugins/example.js"
...
- `client`: Un cliente SDK novacode para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar novacode.
...
```js title=".novacode/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "novacode"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio NovaCode, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que novacode lea archivos `.env`:

```javascript title=".novacode/plugins/env-protection.js"
...
```javascript title=".novacode/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a novacode:

```ts title=".novacode/plugins/custom-tools.ts"
import { type Plugin, tool } from "@novacode-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que novacode puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para novacode junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://novacode.ai/docs/sdk) para obtener más detalles.
...
```ts title=".novacode/plugins/compaction.ts"
import type { Plugin } from "@novacode-ai/plugin"
...
```ts title=".novacode/plugins/custom-compaction.ts"
import type { Plugin } from "@novacode-ai/plugin"
