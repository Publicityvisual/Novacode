...
Con NovaCode puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, NovaCode usa nuestro propio tema `novacode`.
...
NovaCode viene con varios temas integrados.
...
- Quiere que NovaCode coincida con la apariencia de su terminal
...
NovaCode admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/novacode/themes/*.json` o `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.novacode/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.novacode/themes/*.json`
