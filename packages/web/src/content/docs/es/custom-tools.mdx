...
description: Cree herramientas que LLM pueda llamar en novacode.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de novacode como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.novacode/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/novacode/tools/`.
...
```ts title=".novacode/tools/database.ts" {1}
...
```ts title=".novacode/tools/math.ts"
import { tool } from "@novacode-ai/plugin"
...
```ts title=".novacode/tools/bash.ts"
import { tool } from "@novacode-ai/plugin"
...
```ts title=".novacode/tools/project.ts" {8}
import { tool } from "@novacode-ai/plugin"
...
```python title=".novacode/tools/add.py"
import sys
...
```ts title=".novacode/tools/python-add.ts" {10}
import { tool } from "@novacode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".novacode/tools/add.py")
