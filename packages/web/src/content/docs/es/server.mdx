---
title: Servidor
description: Interactuar con el servidor novacode a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `novacode serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente novacode puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `novacode.local` |
...
Configure `OPENCODE_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `novacode`, o configure `OPENCODE_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `novacode serve` como a `novacode web`.
...
Cuando ejecuta `novacode`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor novacode para interactuar con novacode mediante programación.
:::

Esta arquitectura permite que novacode admita múltiples clientes y le permite interactuar con novacode mediante programación.

Puede ejecutar `novacode serve` para iniciar un servidor independiente. Si tienes el
novacode TUI ejecutándose, `novacode serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos NovaCode [IDE](/docs/ide).
...
El servidor novacode expone las siguientes API.
