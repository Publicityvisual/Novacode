...
To debug issues with NovaCode, start by checking the logs and local data it stores on disk.
...
You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `novacode --log-level DEBUG`.

---

## Storage

novacode stores session data and other application data on disk at:
...
To uninstall the NovaCode CLI and remove its related files, run:
...
To remove NovaCode Desktop, uninstall the application through your operating system's app management tools.

---

## Desktop app

NovaCode Desktop runs a local NovaCode server (the `novacode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.

### Quick checks

- Fully quit and relaunch the app.
- If the app shows an error screen, click **Restart** and copy the error details.
- macOS only: `NovaCode` menu -> **Reload Webview** (helps if the UI is blank/frozen).
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (or `~/.config/novacode/novacode.json`)
- **macOS/Linux** (older installs): `~/.local/share/novacode/novacode.jsonc`
...
NovaCode can also load local plugins from disk. Temporarily move these out of the way (or rename the folder) and restart the desktop app:
...
If disabling plugins doesn't help (or a plugin install is stuck), clear the cache so NovaCode can rebuild it.

1. Quit NovaCode Desktop completely.
2. Delete the cache directory:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/novacode`
- **Linux**: delete `~/.cache/novacode` (or run `rm -rf ~/.cache/novacode`)
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.cache\novacode`

3. Restart NovaCode Desktop.

---

### Fix server connection issues

NovaCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
If your `novacode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
...
On Windows, NovaCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.

---

### Windows: General performance issues

If you're experiencing slow performance, file access issues, or terminal problems on Windows, try using [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL provides a Linux environment that works more seamlessly with NovaCode's features.

---

### Notifications not showing

NovaCode Desktop only shows system notifications when:

- notifications are enabled for NovaCode in your OS settings, and
...
1. Quit NovaCode Desktop.
2. Find and delete these files (they live in the NovaCode Desktop app data directory):

- `novacode.settings.dat` (desktop default server URL)
- `novacode.global.dat` and `novacode.workspace.*.dat` (UI state like recent servers/projects)
...
If you're experiencing issues with NovaCode:
...
### NovaCode won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `novacode upgrade`
...
To figure out what models you have access to, run `novacode models`
...
If you encounter API call errors, this may be due to outdated provider packages. novacode dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
2. Restart novacode to reinstall the latest provider packages

This will force novacode to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
novacode will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
