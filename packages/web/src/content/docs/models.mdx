...
Most popular providers are preloaded by default. If you've added the credentials for a provider through the `/connect` command, they'll be available when you start NovaCode.
...
Here are several models that work well with NovaCode, in no particular order. (This is not an exhaustive list nor is it necessarily up to date):
...
Here the full ID is `provider_id/model_id`. For example, if you're using [NovaCode Zen](/docs/zen), you would use `novacode/gpt-5.1-codex` for GPT 5.1 Codex.
...
Many models support multiple variants with different configurations. NovaCode ships with built-in default variants for popular providers.

### Built-in variants

NovaCode ships with default variants for many providers:
...
When NovaCode starts up, it checks for models in the following priority order:

1. The `--model` or `-m` command line flag. The format is the same as in the config file: `provider_id/model_id`.

2. The model list in the NovaCode config.
