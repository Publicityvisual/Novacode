...
Du kan legge til eksterne verktøy i NovaCode ved å bruke _Model Context Protocol_ eller MCP. NovaCode støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [NovaCode Config](https://novacode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="novacode.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/novacode`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
NovaCode håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil NovaCode:
...
Hvis serveren krever autentisering, vil NovaCode be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `novacode mcp auth <server-name>`.
...
```bash
novacode mcp auth my-oauth-server
```
...
```bash
novacode mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil NovaCode lagre tokens sikkert i `~/.local/share/novacode/mcp-auth.json`.
...
novacode mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i NovaCode, sammen med innebygde verktøy. Så du kan administrere dem gjennom NovaCode-konfigurasjonen som alle andre verktøy.
...
```json title="novacode.json" {11, 14-18}
{
  "$schema": "https://novacode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
novacode mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble NovaCode til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver NovaCode-samtale oppført som en separat økt.
