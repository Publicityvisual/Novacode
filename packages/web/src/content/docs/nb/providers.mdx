---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i NovaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

NovaCode bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i NovaCode-konfigurasjonen.
...
i `~/.local/share/novacode/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i NovaCode
konfig.
...
NovaCode Zen er en liste over modeller levert av NovaCode-teamet som har vært
testet og verifisert for å fungere godt med NovaCode. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med NovaCode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `NovaCode Zen` og gå til [novacode.ai/auth](https://novacode.ai/zen).
...
Det fungerer som alle andre leverandører i NovaCode og er helt valgfritt å bruke.

---

## NovaCode Go

NovaCode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av NovaCode-teamet som har vært
testet og verifisert for å fungere godt med NovaCode.

1. Kjør kommandoen `/connect` i TUI, velg `NovaCode Go`, og gå til [novacode.ai/auth](https://novacode.ai/zen).
...
Slik bruker du Amazon Bedrock med NovaCode:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører NovaCode:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `novacode.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i novacode støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere novacode til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at NovaCode skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom NovaCode-konfigurasjonen.
...
for å lagre token i NovaCode auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
NovaCode bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse NovaCode
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`novacode.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `novacode.json`:
...
Slik bruker du GitHub Copilot-abonnementet med NovaCode:
...
Slik bruker du Google Vertex AI med NovaCode:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører NovaCode.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom NovaCode, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session) plugin for automatisk å logge hver NovaCode-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver NovaCode-samtale oppført som en separat økt.
...
Du kan konfigurere NovaCode for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere NovaCode for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere NovaCode for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for NovaCode. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/novacode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med NovaCode:
...
3. Kopier API-nøkkelen for bruk i NovaCode.
...
6. **Viktig**: Før du bruker skymodeller i NovaCode, må du hente modellinformasjonen lokalt:
...
NovaCode Zen er en liste over testede og verifiserte modeller levert av NovaCode-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>NovaCode Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **NovaCode Zen**.

   ```txt
   /connect
   ```

3. Skriv inn NovaCode API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom NovaCode-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom NovaCode-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med NovaCode:
...
Du kan også tilpasse modeller gjennom NovaCode-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med NovaCode. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `novacode.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar NovaCode forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `novacode auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk NovaCode-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i NovaCode-konfigurasjonen.
