---
title: Tjener
description: Samhandle med NovaCode-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `novacode serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en NovaCode-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `novacode.local` |
...
Sett `OPENCODE_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `novacode` som standard, eller sett `OPENCODE_SERVER_USERNAME` for å overstyre det. Dette gjelder både `novacode serve` og `novacode web`.
...
Når du kjører `novacode` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk NovaCode-serveren til å samhandle med NovaCode programmatisk.
:::

Denne arkitekturen lar NovaCode støtte flere klienter og lar deg samhandle med NovaCode programmatisk.

Du kan kjøre `novacode serve` for å starte en frittstående server. Hvis du har
NovaCode TUI kjører, vil `novacode serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av NovaCode [IDE](/docs/ide) plugins.
...
novacode-serveren viser følgende APIer.
