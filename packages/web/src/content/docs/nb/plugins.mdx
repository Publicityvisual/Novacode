...
description: Skriv dine egne programtillegg for å utvide NovaCode.
---

Plugins lar deg utvide NovaCode ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til NovaCode.
...
- `.novacode/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/novacode/node_modules/`.
...
1. Global konfigurasjon (`~/.config/novacode/novacode.json`)
2. Prosjektkonfigurasjon (`novacode.json`)
3. Global plugin-katalog (`~/.config/novacode/plugins/`)
4. Prosjektpluginkatalog (`.novacode/plugins/`)
...
NovaCode kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En NovaCode SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide NovaCode.
...
:::note
Hvis du bruker NovaCode-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre NovaCode fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til NovaCode:
...
`tool`-hjelperen lager et tilpasset verktøy som NovaCode kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for NovaCode sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://novacode.ai/docs/sdk) for detaljer.
