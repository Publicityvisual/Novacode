---
title: CLI
description: NovaCode CLI-alternativer og kommandoer.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NovaCode CLI starter som standard [TUI](/docs/tui) når den kjøres uten noen argumenter.

```bash
novacode
```

Men den godtar også kommandoer som dokumentert på denne siden. Dette lar deg samhandle med NovaCode programmatisk.
...
Start NovaCode-terminalbrukergrensesnittet.
...
Administrer agenter for NovaCode.
...
Koble en terminal til en allerede kjørende NovaCode-backend-server startet via `serve` eller `web` kommandoer.

```bash
novacode attach [url]
```

Dette gjør det mulig å bruke TUI med en ekstern NovaCode-backend. For eksempel:
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `novacode`) |
...
NovaCode drives av leverandørlisten på [Models.dev](https://models.dev), så du kan bruke `novacode auth login` til å konfigurere API-nøkler for enhver leverandør du vil bruke. Dette er lagret i `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Når NovaCode starter opp, laster den leverandørene fra filen med påloggingsinformasjon. Og hvis det er noen nøkler definert i miljøene dine eller en `.env`-fil i prosjektet ditt.
...
Bruk `--refresh`-flagget for å oppdatere den bufrede modelllisten. Dette er nyttig når nye modeller er lagt til en leverandør og du vil se dem i NovaCode.
...
Kjør NovaCode i ikke-interaktiv modus ved å sende en melding direkte.
...
Du kan også koble til en kjørende `novacode serve`-forekomst for å unngå MCP-serveres kaldstarttid ved hver kjøring:
...
| <nobr><code>{"--attach"}</code></nobr>   |      | Koble til en kjørende NovaCode-server (f.eks. http://localhost:4096)                               |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Passord for grunnleggende autentisering (standard: `OPENCODE_SERVER_PASSWORD`)                     |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Brukernavn for grunnleggende autentisering (standard: `OPENCODE_SERVER_USERNAME` eller `novacode`) |
...
Start en headless NovaCode-server for API-tilgang. Sjekk ut [server-dokumentene](/docs/server) for hele HTTP-grensesnittet.

```bash
novacode serve
```

Dette starter en HTTP-server som gir API tilgang til NovaCode-funksjonalitet uten TUI-grensesnittet. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `novacode`).
...
Vis tokenbruk og kostnadsstatistikk for NovaCode-øktene dine.
...
Importer øktdata fra en JSON-fil eller en NovaCode share-URL.
...
Start en headless NovaCode-server med et webgrensesnitt.

```bash
novacode web
```

Dette starter en HTTP-server og åpner en nettleser for å få tilgang til NovaCode via et nettgrensesnitt. Sett `OPENCODE_SERVER_PASSWORD` for å aktivere HTTP grunnleggende autentisering (brukernavn er standard til `novacode`).
...
Avinstaller NovaCode og fjern alle relaterte filer.
...
Oppdaterer NovaCode til siste versjon eller en spesifikk versjon.
...
NovaCode CLI bruker følgende globale flagg.
...
NovaCode kan konfigureres ved hjelp av miljøvariabler.
...
| `OPENCODE_SERVER_USERNAME`            | streng | Overstyr grunnleggende autentiseringsbrukernavn (standard `novacode`) |
