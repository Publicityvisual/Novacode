...
Agentferdigheter lar NovaCode oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
novacode søker etter disse stedene:

- Prosjektkonfigurasjon: `.novacode/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/novacode/skills/<name>/SKILL.md`
...
For prosjektlokale stier går novacode opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.novacode/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.novacode/skills/git-release/SKILL.md` slik:
...
novacode viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `novacode.json`:
...
**For innebygde agenter** (i `novacode.json`):
