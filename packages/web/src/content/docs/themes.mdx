...
With NovaCode you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, NovaCode uses our own `novacode` theme.
...
NovaCode comes with several built-in themes.
...
- Want NovaCode to match their terminal's appearance
...
NovaCode supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/novacode/themes/*.json` or `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.novacode/themes/*.json`
4. **Current working directory** - Defined in `./.novacode/themes/*.json`
