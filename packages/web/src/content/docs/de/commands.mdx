...
Erstellen Sie `.novacode/commands/test.md`:

```md title=".novacode/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die NovaCode-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem NovaCode [config](/docs/config):

```json title="novacode.jsonc" {4-12}
{
...
- Global: `~/.config/novacode/commands/`
- Pro Projekt: `.novacode/commands/`

```markdown title="~/.config/novacode/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".novacode/commands/component.md"
---
...
```md title=".novacode/commands/create-file.md"
---
...
```md title=".novacode/commands/analyze-coverage.md"
---
...
```md title=".novacode/commands/review-changes.md"
---
...
```md title=".novacode/commands/review-component.md"
---
...
NovaCode enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
