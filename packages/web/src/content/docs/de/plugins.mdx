---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um NovaCode zu erweitern.
---

Mit Plugins können Sie NovaCode erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von NovaCode zu ändern.
...
- `.novacode/plugins/` – Plugins auf Projektebene
- `~/.config/novacode/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/novacode/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/novacode/novacode.json`)
2. Projektkonfiguration (`novacode.json`)
3. Globales Plugin-Verzeichnis (`~/.config/novacode/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.novacode/plugins/`)
...
NovaCode führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein NovaCode-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie NovaCode erweitern können.
...
:::note
Wenn Sie die NovaCode-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass NovaCode `.env`-Dateien liest:
...
Plugins können NovaCode auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das NovaCode aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für NovaCode zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://novacode.ai/docs/sdk).
