---
title: Regeln
description: Lege eigene Anweisungen fuer novacode fest.
---

Du kannst novacode ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in novacode den Befehl `/init` aus.
...
So kann novacode sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht novacode sie zu erweitern.
...
novacode kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/novacode/AGENTS.md` speichern.
Diese gelten in allen novacode-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt NovaCode dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/novacode/AGENTS.md` existiert)
...
Beim Start sucht novacode in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/novacode/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `novacode.json` oder in `~/.config/novacode/novacode.json` angeben.
...
novacode parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit novacode.json

Empfohlen ist das `instructions`-Feld in der `novacode.json`:
...
Du kannst novacode explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `novacode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
