...
Mit NovaCode kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet NovaCode das `novacode`-Theme.
...
NovaCode liefert mehrere eingebaute Themes mit.
...
- NovaCode optisch ans Terminal anpassen wollen
...
NovaCode unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/novacode/themes/*.json` oder `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.novacode/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.novacode/themes/*.json`
