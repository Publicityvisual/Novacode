---
title: CLI
description: NovaCode CLI Optionen und Befehle.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Der NovaCode CLI startet standardmäßig das [TUI](/docs/tui), wenn er ohne Argumente ausgeführt wird.

```bash
novacode
```

Es akzeptiert aber auch Befehle, wie auf dieser Seite dokumentiert. Dadurch können Sie programmgesteuert mit NovaCode interagieren.

```bash
novacode run "Explain how closures work in JavaScript"
```
...
Starten Sie die Terminalbenutzeroberfläche NovaCode.
...
Der NovaCode CLI verfügt außerdem über die folgenden Befehle.

---

### agent

Agenten für NovaCode verwalten.

```bash
novacode agent [command]
```
...
Schließen Sie ein Terminal an einen bereits laufenden NovaCode-Backend-Server an, der über die Befehle `serve` oder `web` gestartet wurde.

```bash
novacode attach [url]
```

Dies ermöglicht die Verwendung von TUI mit einem Remote-Backend NovaCode. Zum Beispiel:

```bash
# Start the backend server for web/mobile access
novacode web --port 4096 --hostname 0.0.0.0

# In another terminal, attach the TUI to the running backend
novacode attach http://10.20.30.40:4096
```
...
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `novacode`) |
...
NovaCode wird von der Anbieterliste bei [Models.dev](https://models.dev) unterstützt, sodass Sie `novacode auth login` verwenden können, um API-Schlüssel für jeden Anbieter zu konfigurieren, den Sie verwenden möchten. Dies wird in `~/.local/share/novacode/auth.json` gespeichert.

```bash
novacode auth login
```

Wenn NovaCode startet, lädt es den Anbieter aus der Anmeldeinformationsdatei. Und ob in Ihren Umgebungen Schlüssel sind oder in Ihrem Projekt eine `.env`-Datei vorhanden ist.
...
```bash
novacode mcp auth list
```

Oder nutzen Sie die Kurzversion.

```bash
novacode mcp auth ls
```
...
Verwenden Sie das Flag `--refresh`, um die zwischengespeicherte Modellliste zu aktualisieren. Dies ist nützlich, wenn einem Anbieter neue Modelle hinzugefügt wurden und Sie diese in NovaCode sehen möchten.

```bash
novacode models --refresh
```
...
Führen Sie NovaCode im nicht interaktiven Modus aus, indem Sie direkt eine Eingabeaufforderung übergeben.

```bash
novacode run [message..]
```
...
```bash "novacode run"
novacode run Explain the use of context in Go
```

Sie können auch eine Verbindung zu einer laufenden `novacode serve`-Instanz herstellen, um bei jedem Lauf Kaltstartzeiten für den MCP-Server zu vermeiden:
...
novacode run --attach http://localhost:4096 "Explain async/await in JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |      | An einen laufenden NovaCode-Server anschließen (z.B. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p` | Basic-Auth-Passwort (standardmäßig `OPENCODE_SERVER_PASSWORD`)                                      |
| <nobr><code>{"--username"}</code></nobr> | `-u` | Basic-Auth-Benutzername (standardmäßig `OPENCODE_SERVER_USERNAME` oder `novacode`)                  |
...
Starten Sie einen Headless-NovaCode-Server für den API-Zugriff. Sehen Sie sich die [Server-Dokumentation](/docs/server) für die vollständige HTTP-Schnittstelle an.

```bash
novacode serve
```

Dadurch wird ein HTTP-Server gestartet, der API-Zugriff auf NovaCode-Funktionalität ohne die TUI-Schnittstelle bietet. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `novacode`).
...
NovaCode-Sitzungen verwalten.

```bash
novacode session [command]
```
...
Listen Sie alle NovaCode-Sitzungen auf.

```bash
novacode session list
```
...
Zeigen Sie Token-Nutzungs- und Kostenstatistiken für Ihre NovaCode-Sitzungen an.
...
Importieren Sie Sitzungsdaten aus einer JSON-Datei oder einer NovaCode-Freigabe-URL.

```bash
novacode import <file>
```

Sie können aus einer lokalen Datei oder einer NovaCode-Freigabe-URL importieren.

```bash
novacode import session.json
novacode import https://opncd.ai/s/abc123
```
...
Starten Sie einen Headless-NovaCode-Server mit einer Weboberfläche.

```bash
novacode web
```

Dadurch wird ein HTTP-Server gestartet und ein Webbrowser geöffnet, um über eine Webschnittstelle auf NovaCode zuzugreifen. Legen Sie `OPENCODE_SERVER_PASSWORD` fest, um die HTTP-Basisauthentifizierung zu aktivieren (Benutzername ist standardmäßig `novacode`).
...
Deinstallieren Sie NovaCode und entfernen Sie alle zugehörigen Dateien.
...
Aktualisiert NovaCode auf die neueste Version oder eine bestimmte Version.

```bash
novacode upgrade [target]
```
...
Der NovaCode CLI akzeptiert die folgenden globalen Flags.
...
NovaCode kann mithilfe von Umgebungsvariablen konfiguriert werden.
...
| `OPENCODE_SERVER_USERNAME`            | Zeichenfolge    | Benutzernamen für die Basisauthentifizierung überschreiben (Standard `novacode`) |
