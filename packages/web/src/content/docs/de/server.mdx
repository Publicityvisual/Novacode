---
title: Server
description: Nutze den novacode-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `novacode serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein novacode-Client nutzen kann.
...
Der Benutzername ist standardmaessig `novacode`, kann aber mit `OPENCODE_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `novacode serve` und `novacode web`.
...
Wenn du `novacode` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den novacode-Server, um novacode programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `novacode serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `novacode serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die NovaCode-[IDE](/docs/ide)-Plugins.
...
Der novacode-Server stellt folgende APIs bereit.
