...
Es gibt zwei Arten von Agenten in NovaCode; Primäragenten und Subagenten.
...
NovaCode verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `novacode.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet NovaCode modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale NovaCode-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer NovaCode-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [NovaCode Zen](/docs/zen) verwenden, würden Sie `novacode/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `novacode models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/anomalyco/novacode).
:::
