...
Agent Skills erlauben NovaCode, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
NovaCode sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft NovaCode vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.novacode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.novacode/skills/git-release/SKILL.md` zum Beispiel so:
...
NovaCode listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `novacode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `novacode.json`):
