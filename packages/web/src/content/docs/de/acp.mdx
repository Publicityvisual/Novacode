...
description: Verwenden Sie NovaCode in jedem ACP-kompatiblen Editor.
---

NovaCode unterstützt [Agent Client Protocol](https://agentclientprotocol.com) oder (ACP), sodass Sie es direkt in kompatiblen Editoren und IDEs verwenden können.
...
Um NovaCode über ACP zu verwenden, konfigurieren Sie Ihren Editor für die Ausführung des Befehls `novacode acp`.

Der Befehl startet NovaCode als ACP-kompatiblen Unterprozess, der über stdio über JSON-RPC mit Ihrem Editor kommuniziert.
...
Installieren Sie NovaCode aus der [Zed-ACP-Registry](https://zed.dev/docs/ai/external-agents#registry), indem Sie `zed: acp registry` in der Befehlspalette ausführen.

Wenn Sie stattdessen eine benutzerdefinierte NovaCode-Programmdatei verwenden möchten, fügen Sie sie Ihrer [Zed](https://zed.dev)-Konfiguration (`~/.config/zed/settings.json`) hinzu:
...
Um es zu öffnen, verwenden Sie den neuen Agenten „NovaCode“ in der AI Chat Agent Selector.
...
Um NovaCode als ACP-Agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim) zu verwenden, fügen Sie Folgendes zu Ihrer Neovim-Konfiguration hinzu:
...
Diese Konfiguration richtet CodeCompanion so ein, dass NovaCode als ACP-Agent für den Chat verwendet wird.
...
NovaCode funktioniert über ACP genauso wie im Terminal. Alle Funktionen werden unterstützt:
...
- MCP-Server, die in Ihrer NovaCode-Konfiguration konfiguriert sind
