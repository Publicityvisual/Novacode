---
title: Policies
description: Control which configured resources NovaCode may use.
---

Policies control whether NovaCode may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `novacode.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether NovaCode may use a resource such as an LLM provider.
...
NovaCode currently supports one policy action:
