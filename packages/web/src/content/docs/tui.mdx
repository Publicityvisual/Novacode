---
title: TUI
description: Using the NovaCode terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NovaCode provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running NovaCode starts the TUI for the current directory.
...
When using the NovaCode TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to NovaCode. Allows you to select from available providers and add their API keys.
...
Exit NovaCode. _Aliases_: `/quit`, `/q`
...
This is separate from `novacode.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long NovaCode waits after the leader key. Defaults to `2000`.
...
- `sound_pack` - Sound pack ID to use. Defaults to `novacode.default`.
