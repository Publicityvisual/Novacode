...
Создайте `.novacode/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию novacode или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем novacode [config](/docs/config):
...
- Глобальный: `~/.config/novacode/commands/`
- Для каждого проекта: `.novacode/commands/`
...
novacode включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
