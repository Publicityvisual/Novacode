...
مع NovaCode يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم NovaCode سمتنا `novacode`.
...
يأتي NovaCode مع عدة سمات مدمجة.
...
- يريدون أن يطابق NovaCode مظهر terminal لديهم
...
يدعم NovaCode نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/novacode/themes/*.json` أو `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.novacode/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.novacode/themes/*.json`
