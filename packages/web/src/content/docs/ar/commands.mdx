...
أنشئ الملف `.novacode/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات NovaCode أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ NovaCode:
...
- على مستوى النظام: `~/.config/novacode/commands/`
- لكل مشروع: `.novacode/commands/`
...
يتضمن novacode عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
