---
title: المزوّدون
description: استخدام أي موفّر LLM في NovaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم NovaCode ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات NovaCode.
...
في `~/.local/share/novacode/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات NovaCode.
...
NovaCode Zen هي قائمة نماذج يوفّرها فريق NovaCode وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع NovaCode. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع NovaCode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر novacode، ثم انتقل إلى [novacode.ai/auth](https://novacode.ai/auth).
...
يعمل مثل أي مزوّد آخر في NovaCode واستخدامه اختياري بالكامل.

---

## NovaCode Go

NovaCode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق NovaCode، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع NovaCode.

1. شغّل الأمر `/connect` في TUI، واختر `NovaCode Go`، ثم انتقل إلى [novacode.ai/auth](https://novacode.ai/zen).
...
لاستخدام Amazon Bedrock مع NovaCode:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل novacode:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `novacode.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل NovaCode غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين novacode لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل novacode بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات novacode.
...
تخزين الرمز في مساحة تخزين مصادقة novacode.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم NovaCode نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر NovaCode
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`novacode.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth).

##### التهيئة

خصّص عبر `novacode.json`:
...
لاستخدام اشتراك GitHub Copilot مع novacode:
...
لاستخدام Google Vertex AI مع NovaCode:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر novacode، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session) لتسجيل كل محادثة NovaCode تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة NovaCode مدرجة كجلسة مستقلة.
...
يمكنك ضبط novacode لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط novacode لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط novacode لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع NovaCode. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/novacode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع NovaCode:
...
3. انسخ مفتاح API لاستخدامه في NovaCode.
...
6. **مهم**: قبل استخدام نماذج السحابة في NovaCode، يجب سحب معلومات النموذج محليا:
...
NovaCode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق NovaCode. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>NovaCode Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **NovaCode Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ NovaCode.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات novacode.
...
5. يمكنك أيضا تخصيصها عبر إعدادات novacode. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات novacode. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع novacode. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `novacode.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ NovaCode بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `novacode auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات novacode وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات novacode.
