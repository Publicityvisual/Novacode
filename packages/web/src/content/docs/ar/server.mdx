---
title: الخادم
description: تفاعل مع خادم novacode عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `novacode serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل novacode استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `novacode.local` |
...
عيّن `OPENCODE_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `novacode`، أو عيّن `OPENCODE_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `novacode serve` و `novacode web`.
...
عند تشغيل `novacode` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم novacode للتفاعل مع novacode برمجيا.
:::

تتيح هذه البنية لـ novacode دعم عدة عملاء وتمكّنك من التفاعل مع novacode برمجيا.

يمكنك تشغيل `novacode serve` لبدء خادم مستقل. إذا كانت واجهة novacode في terminal (TUI)
قيد التشغيل، فسيبدأ `novacode serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات NovaCode لـ [IDE](/docs/ide).
...
يعرض خادم novacode واجهات API التالية.
