---
title: CLI
description: أوامر وخيارات CLI في NovaCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يبدأ CLI الخاص بـ NovaCode افتراضيا واجهة [TUI](/docs/tui) عند تشغيله بدون أي معاملات.

```bash
novacode
```

كما يدعم أيضا أوامر موثقة في هذه الصفحة، ما يتيح لك التفاعل مع NovaCode برمجيا.
...
بدء واجهة المستخدم terminal (TUI) الخاصة بـ NovaCode.
...
يتضمن CLI الخاص بـ NovaCode أيضا الأوامر التالية.

---

### agent

إدارة وكلاء NovaCode.
...
إرفاق terminal بخادم الواجهة الخلفية لـ NovaCode قيد التشغيل بالفعل، والذي تم تشغيله عبر الأمرين `serve` أو `web`.

```bash
novacode attach [url]
```

يتيح ذلك استخدام واجهة TUI مع واجهة خلفية لـ NovaCode تعمل عن بعد. على سبيل المثال:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `novacode`) |
...
يعتمد NovaCode على قائمة المزودين في [Models.dev](https://models.dev)، لذا يمكنك استخدام `novacode auth login` لتهيئة مفاتيح API لأي مزود ترغب باستخدامه. يتم حفظ ذلك في `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

عند تشغيل NovaCode يقوم بتحميل المزودين من ملف بيانات الاعتماد، وكذلك أي مفاتيح معرّفة في متغيرات البيئة لديك أو في ملف `.env` ضمن مشروعك.
...
استخدم الراية `--refresh` لتحديث قائمة النماذج المخزنة مؤقتا. يفيد ذلك عند إضافة نماذج جديدة إلى مزود وتريد رؤيتها في NovaCode.
...
تشغيل novacode في وضع غير تفاعلي عبر تمرير موجه مباشرة.
...
يمكنك أيضا الإرفاق بمثيل `novacode serve` قيد التشغيل لتجنّب زمن الإقلاع البارد لخوادم MCP في كل تشغيل:
...
| <nobr><code>{"--attach"}</code></nobr>   |         | الإرفاق بخادم novacode قيد التشغيل (مثل http://localhost:4096)                    |
| <nobr><code>{"--password"}</code></nobr> | `-p`    | كلمة مرور المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_PASSWORD`)                |
| <nobr><code>{"--username"}</code></nobr> | `-u`    | اسم مستخدم المصادقة الأساسية (الافتراضي `OPENCODE_SERVER_USERNAME` أو `novacode`) |
...
بدء خادم NovaCode بدون واجهة للوصول عبر API. راجع [توثيق الخادم](/docs/server) لواجهة HTTP الكاملة.

```bash
novacode serve
```

يشغّل هذا خادم HTTP يوفّر وصولا عبر API لوظائف novacode دون واجهة TUI. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `novacode`).
...
عرض إحصاءات استخدام الرموز والتكلفة لجلسات NovaCode لديك.
...
استيراد بيانات الجلسة من ملف JSON أو رابط مشاركة NovaCode.

```bash
novacode import <file>
```

يمكنك الاستيراد من ملف محلي أو من رابط مشاركة NovaCode.
...
بدء خادم NovaCode بدون واجهة مع واجهة ويب.

```bash
novacode web
```

يشغّل هذا خادم HTTP ويفتح متصفح ويب للوصول إلى NovaCode عبر واجهة ويب. اضبط `OPENCODE_SERVER_PASSWORD` لتفعيل مصادقة HTTP الأساسية (اسم المستخدم الافتراضي `novacode`).
...
إلغاء تثبيت NovaCode وإزالة جميع الملفات ذات الصلة.
...
تحديث novacode إلى أحدث إصدار أو إلى إصدار محدد.
...
يدعم سطر أوامر novacode الخيارات العامة التالية.
...
يمكن تهيئة NovaCode باستخدام متغيرات البيئة.
...
| `OPENCODE_SERVER_USERNAME`            | string  | تجاوز اسم مستخدم المصادقة الأساسية (الافتراضي `novacode`) |
