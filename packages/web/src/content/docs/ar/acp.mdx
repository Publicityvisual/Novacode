---
title: دعم ACP
description: استخدم NovaCode في أي محرر متوافق مع ACP.
---

يدعم NovaCode بروتوكول [Agent Client Protocol](https://agentclientprotocol.com) أو (ACP)، مما يتيح لك استخدامه مباشرة في المحررات وبيئات التطوير المتكاملة (IDEs) المتوافقة.
...
لاستخدام NovaCode عبر ACP، اضبط محررك ليشغّل الأمر `novacode acp`.

يشغّل هذا الأمر NovaCode كعملية فرعية متوافقة مع ACP تتواصل مع محررك عبر JSON-RPC باستخدام stdio.
...
ثبّت NovaCode من [سجل ACP في Zed](https://zed.dev/docs/ai/external-agents#registry) عبر تشغيل `zed: acp registry` من لوحة الأوامر.

لاستخدام ملف NovaCode تنفيذي مخصص بدلاً من ذلك، أضفه إلى إعدادات [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
لفتحه، اختر الوكيل الجديد 'NovaCode' من محدد الوكلاء في AI Chat.
...
لاستخدام NovaCode كوكيل ACP في [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim)، أضف التالي إلى إعدادات Neovim:
...
يضبط هذا الإعداد CodeCompanion لاستخدام NovaCode كوكيل ACP للدردشة.
...
يعمل NovaCode عبر ACP بالطريقة نفسها التي يعمل بها في terminal. جميع الميزات مدعومة:
...
- خوادم MCP المضبوطة في إعدادات NovaCode
