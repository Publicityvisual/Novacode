...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في novacode.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في novacode مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.novacode/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/novacode/tools/`.
