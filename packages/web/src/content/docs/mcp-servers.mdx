...
You can add external tools to NovaCode using the _Model Context Protocol_, or MCP. NovaCode supports both local and remote servers.
...
You can define MCP servers in your [NovaCode Config](https://novacode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/novacode` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
NovaCode automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, NovaCode will:
...
If the server requires authentication, NovaCode will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `novacode mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, NovaCode will store the tokens securely in `~/.local/share/novacode/mcp-auth.json`.
...
Your MCPs are available as tools in NovaCode, alongside built-in tools. So you can manage them through the NovaCode config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect NovaCode to your Sentry account.
