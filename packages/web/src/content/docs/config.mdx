---
title: Config
description: Using the NovaCode JSON config.
---

You can configure NovaCode using a JSON config file.

---

## Format

NovaCode supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="novacode.jsonc"
{
...
1. **Remote config** (from `.well-known/novacode`) - organizational defaults
2. **Global config** (`~/.config/novacode/novacode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`novacode.json` in project) - project-specific settings
5. **`.novacode` directories** - agents, commands, plugins
6. **Inline config** (`OPENCODE_CONFIG_CONTENT` env var) - runtime overrides
7. **Managed config files** (`/Library/Application Support/novacode/` on macOS) - admin-controlled
...
:::note
The `.novacode` and `~/.config/novacode` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/novacode` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/novacode"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global NovaCode config in `~/.config/novacode/novacode.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.

For TUI-specific settings, use `~/.config/novacode/tui.json`.
...
Add `novacode.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When NovaCode starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
modes, and plugins just like the standard `.novacode` directory, and should
...
The custom directory is loaded after the global config and `.novacode` directories, so it **can override** their settings.
...
Drop an `novacode.json` or `novacode.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/novacode/` |
| Linux    | `/etc/novacode/`                         |
| Windows  | `%ProgramData%\novacode`                 |
...
On macOS, NovaCode reads managed preferences from the `ai.novacode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

NovaCode checks these paths:

1. `/Library/Managed Preferences/<user>/ai.novacode.managed.plist`
2. `/Library/Managed Preferences/ai.novacode.managed.plist`

The plist keys map directly to `novacode.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.novacode.managed` PayloadType. The NovaCode config keys go directly in the payload dict:
...
<dict>
  ...
  <array>
    <dict>
      <key>PayloadType</key>
      <string>ai.novacode.managed</string>
      <key>PayloadIdentifier</key>
      <string>com.example.novacode.config</string>
      <key>PayloadUUID</key>
  ...
  <key>PayloadIdentifier</key>
  <string>com.example.novacode</string>
  <key>PayloadUUID</key>
  <string>GENERATE-YOUR-OWN-UUID</string>
  <key>PayloadVersion</key>
  <integer>1</integer>
...
The server/runtime config schema is defined in [**`novacode.ai/config.json`**](https://novacode.ai/config.json).

TUI config uses [**`novacode.ai/tui.json`**](https://novacode.ai/tui.json).
...
Legacy `theme`, `keybinds`, and `tui` keys in `novacode.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `novacode serve` and `novacode web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your NovaCode server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `novacode.local`. Useful for running multiple instances on the same network.
...
If not specified, NovaCode will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your NovaCode config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, NovaCode tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny NovaCode actions on configured resources. Currently, policies can control which providers NovaCode may use.
...
NovaCode normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, NovaCode omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/novacode/agents/` or `.novacode/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, NovaCode will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`novacode run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/novacode/commands/` or `.novacode/commands/`. [Learn more here](/docs/commands).
...
NovaCode uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
NovaCode will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, novacode **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend NovaCode with custom tools, hooks, and integrations.

Place plugin files in `.novacode/plugins/` or `~/.config/novacode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="novacode.json"
{
  "$schema": "https://novacode.ai/config.json",
  "plugin": ["novacode-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict NovaCode to only use specific providers rather than disabling them one by one.
