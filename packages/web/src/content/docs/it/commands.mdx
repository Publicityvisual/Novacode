...
Crea `.novacode/commands/test.md`:
...
Puoi aggiungere comandi personalizzati tramite la config di NovaCode o creando file markdown nella directory `commands/`.

---

### JSON

Usa l'opzione `command` nella [config](/docs/config) di NovaCode:
...
- Globale: `~/.config/novacode/commands/`
- Per progetto: `.novacode/commands/`
...
novacode include diversi comandi integrati come `/init`, `/undo`, `/redo`, `/share`, `/help`; [scopri di piu](/docs/tui#commands).
