...
Puoi aggiungere strumenti esterni a NovaCode usando il _Model Context Protocol_, o MCP. NovaCode supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [NovaCode Config](https://novacode.ai/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/novacode`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
NovaCode gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, NovaCode:
...
Se il server richiede autenticazione, NovaCode ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `novacode mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, NovaCode memorizzera' i token in modo sicuro in `~/.local/share/novacode/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in NovaCode insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione NovaCode come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare NovaCode al tuo account Sentry.
