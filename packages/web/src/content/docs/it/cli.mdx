---
title: CLI
description: Opzioni e comandi della CLI di NovaCode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

La CLI di NovaCode, per impostazione predefinita, avvia la [TUI](/docs/tui) quando viene eseguita senza argomenti.

```bash
novacode
```

Ma accetta anche i comandi documentati in questa pagina. Questo ti permette di interagire con NovaCode in modo programmatico.
...
Avvia l'interfaccia testuale di NovaCode (interfaccia utente da terminale).
...
La CLI di NovaCode include anche i seguenti comandi.

---

### agent

Gestisci gli agenti per NovaCode.
...
Collega un terminale a un backend server di NovaCode già in esecuzione avviato tramite i comandi `serve` o `web`.

```bash
novacode attach [url]
```

Questo consente di usare la TUI con un backend NovaCode remoto. Per esempio:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `novacode`) |
...
NovaCode si basa sull'elenco provider di [Models.dev](https://models.dev), quindi puoi usare `novacode auth login` per configurare le API key per qualunque provider tu voglia usare. Le credenziali vengono salvate in `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Quando NovaCode si avvia, carica i provider dal file delle credenziali e, se presenti, anche eventuali key definite nell'ambiente o in un file `.env` nel progetto.
...
Usa `--refresh` per aggiornare l'elenco modelli in cache. È utile quando nuovi modelli vengono aggiunti a un provider e vuoi vederli in NovaCode.
...
Esegui novacode in modalità non interattiva passando un prompt direttamente.
...
Puoi anche collegarti a una istanza `novacode serve` già in esecuzione per evitare i cold boot dei server MCP ad ogni esecuzione:
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Attach a un server novacode in esecuzione (es. http://localhost:4096)                           |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Password per l'autenticazione di base (predefinita: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Nome utente per l'autenticazione di base (predefinito: `OPENCODE_SERVER_USERNAME` o `novacode`) |
...
Avvia un server NovaCode headless per accesso via API. Vedi le [server docs](/docs/server) per l'interfaccia HTTP completa.

```bash
novacode serve
```

Avvia un server HTTP che espone accesso API alle funzionalità di novacode senza la TUI. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `novacode`).
...
Elenca tutte le sessioni NovaCode.
...
Mostra statistiche di utilizzo token e costo per le sessioni NovaCode.
...
Importa i dati di sessione da un file JSON o da un URL di condivisione NovaCode.

```bash
novacode import <file>
```

Puoi importare da un file locale o da un URL di condivisione NovaCode.
...
Avvia un server NovaCode headless con interfaccia web.

```bash
novacode web
```

Avvia un server HTTP e apre un browser per accedere a NovaCode tramite interfaccia web. Imposta `OPENCODE_SERVER_PASSWORD` per abilitare HTTP basic auth (username di default `novacode`).
...
Disinstalla NovaCode e rimuove tutti i file correlati.
...
Aggiorna novacode all'ultima versione o a una versione specifica.
...
La CLI di novacode accetta i seguenti flag globali.
...
NovaCode può essere configurato tramite variabili d'ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Sovrascrive lo username basic auth (default `novacode`)     |
