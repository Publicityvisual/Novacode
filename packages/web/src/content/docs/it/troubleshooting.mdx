...
Per diagnosticare problemi con NovaCode, inizia controllando i log e i dati locali che salva su disco.
...
Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `novacode --log-level DEBUG`.

---

## Archiviazione

novacode salva i dati delle sessioni e altri dati dell'applicazione su disco in:
...
NovaCode Desktop esegue in background un server locale di NovaCode (il sidecar `novacode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `NovaCode` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/novacode/novacode.jsonc`
...
NovaCode puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che NovaCode possa ricostruirla.

1. Chiudi completamente NovaCode Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/novacode`
- **Linux**: elimina `~/.cache/novacode` (oppure esegui `rm -rf ~/.cache/novacode`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\novacode`

3. Riavvia NovaCode Desktop.

---

### Risolvi problemi di connessione al server

NovaCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `novacode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
...
Su Windows, NovaCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di NovaCode.

---

### Notifiche non visualizzate

NovaCode Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per NovaCode nelle impostazioni del sistema operativo, e
...
1. Chiudi NovaCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app NovaCode Desktop):

- `novacode.settings.dat` (desktop default server URL)
- `novacode.global.dat` e `novacode.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con NovaCode:
...
Per capire a quali modelli hai accesso, esegui `novacode models`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. novacode installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
2. Riavvia novacode per reinstallare i pacchetti provider piu recenti

Questo forzera novacode a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
novacode rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
