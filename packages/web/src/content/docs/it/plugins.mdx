---
title: Plugin
description: Scrivi plugin per estendere NovaCode.
---

I plugin ti permettono di estendere NovaCode agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di NovaCode.
...
- `.novacode/plugins/` - plugin a livello progetto
- `~/.config/novacode/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/novacode/node_modules/`.
...
1. Config globale (`~/.config/novacode/novacode.json`)
2. Config di progetto (`novacode.json`)
3. Directory plugin globale (`~/.config/novacode/plugins/`)
4. Directory plugin di progetto (`.novacode/plugins/`)
...
NovaCode esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di novacode per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere novacode.
...
:::note
Se usi l'app desktop di NovaCode, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a novacode di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a novacode:
...
L'helper `tool` crea uno strumento personalizzato che novacode può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in novacode insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://novacode.ai/docs/sdk) per i dettagli.
