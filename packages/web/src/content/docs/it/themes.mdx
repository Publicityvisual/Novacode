...
Con NovaCode puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, NovaCode usa il tema `novacode`.
...
NovaCode include diversi temi integrati.
...
- Vuole che NovaCode corrisponda all'aspetto del terminale
...
NovaCode supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/novacode/themes/*.json` o `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.novacode/themes/*.json`
4. **Directory di lavoro corrente** - in `./.novacode/themes/*.json`
