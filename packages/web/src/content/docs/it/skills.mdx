...
Le skill degli agenti permettono a NovaCode di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
NovaCode cerca in queste posizioni:

- Config di progetto: `.novacode/skills/<name>/SKILL.md`
- Config globale: `~/.config/novacode/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, NovaCode risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.novacode/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.novacode/skills/git-release/SKILL.md` cosi':
...
NovaCode elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `novacode.json`:
...
**Per agenti integrati** (in `novacode.json`):
