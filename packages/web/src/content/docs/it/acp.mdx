---
title: Supporto ACP
description: Usa NovaCode in qualunque editor compatibile con ACP.
---

NovaCode supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare NovaCode via ACP, configura l'editor in modo che esegua il comando `novacode acp`.

Il comando avvia NovaCode come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Installa NovaCode dal [registro ACP di Zed](https://zed.dev/docs/ai/external-agents#registry) eseguendo `zed: acp registry` nella palette dei comandi.

Per usare invece un eseguibile NovaCode personalizzato, aggiungilo alla configurazione di [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Per aprirlo, seleziona il nuovo agente 'NovaCode' nel selettore agenti di AI Chat.
...
Per usare NovaCode come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare NovaCode come agente ACP per la chat.
...
NovaCode funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione NovaCode
