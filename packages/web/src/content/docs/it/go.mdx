...
NovaCode Go è un abbonamento a basso costo da **10 $/mese** che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in NovaCode. Ti abboni a NovaCode Go e
...
NovaCode Go ti dà accesso a questi modelli a **10 $/mese**.

---

## Come funziona

NovaCode Go funziona come qualsiasi altro provider in NovaCode.

1. Accedi a **<a href={console}>NovaCode Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `NovaCode Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a NovaCode Go.
:::
...
NovaCode Go è progettato per [NovaCode](https://novacode.ai) e altri agenti di programmazione
...
3. Inviare un ID di sessione stabile in `x-novacode-session` per ogni conversazione, in modo da consentirci di ottimizzare il routing e
   il caching dei prompt.

### Client convalidati

Oltre a NovaCode, è stato verificato che i seguenti client funzionino correttamente
con NovaCode Go. Tuttavia, non garantiamo che continueranno a funzionare in futuro.

| Client            | Supporto delle sessioni                                                                                                                                                                                                                                                      |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | Le build contenenti la [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) inviano l'header nelle richieste NovaCode principali e ausiliarie. La correzione è stata unita dopo la v0.21.0; quella versione da sola non la include.                        |
...
| **ZCode**         | Go riconosce il suo header di sessione nativo. La nostra [richiesta per `x-novacode-session`](https://github.com/zai-org/feedback/issues/492) è ancora aperta, ma non è più necessario inviare proprio quell'header.                                                         |
| **Pi**            | Le build attuali inviano le informazioni di sessione per NovaCode. Aggiorna le installazioni meno recenti.                                                                                                                                                                   |
...
| **Kilo Code CLI** | Le build contenenti la [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) ripristinano gli header di sessione NovaCode. Questa correzione riguarda la CLI, non l'estensione VS Code. Consulta l'[issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
Il [model id](/docs/config/#models) nella tua NovaCode config
utilizza il formato `novacode-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`novacode-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato NovaCode Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con NovaCode.
