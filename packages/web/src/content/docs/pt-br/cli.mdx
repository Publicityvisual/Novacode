---
title: CLI
description: Opções e comandos da CLI do novacode.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

A CLI do novacode, por padrão, inicia o [TUI](/docs/tui) quando executada sem argumentos.

```bash
novacode
```

Mas também aceita comandos conforme documentado nesta página. Isso permite que você interaja com o novacode programaticamente.
...
Inicie a interface de usuário do terminal do novacode.
...
A CLI do novacode também possui os seguintes comandos.

---

### agent

Gerencie agentes para o novacode.
...
Anexe um terminal a um servidor backend do novacode já em execução, iniciado via comandos `serve` ou `web`.

```bash
novacode attach [url]
```

Isso permite usar o TUI com um backend novacode remoto. Por exemplo:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `novacode`) |
...
O novacode é alimentado pela lista de provedores em [Models.dev](https://models.dev), então você pode usar `novacode auth login` para configurar chaves de API para qualquer provedor que você gostaria de usar. Isso é armazenado em `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Quando o novacode é iniciado, ele carrega os provedores do arquivo de credenciais. E se houver chaves definidas em seus ambientes ou em um arquivo `.env` em seu projeto.
...
Use a flag `--refresh` para atualizar a lista de modelos em cache. Isso é útil quando novos modelos foram adicionados a um provedor e você deseja vê-los no novacode.
...
Execute o novacode em modo não interativo passando um prompt diretamente.
...
```bash "novacode run"
novacode run Explique o uso de context em Go
```

Você também pode se anexar a uma instância em execução do `novacode serve` para evitar tempos de inicialização a frio do servidor MCP em cada execução:
...
novacode run --attach http://localhost:4096 "Explique async/await em JavaScript"
```
...
| <nobr><code>{"--attach"}</code></nobr>   |       | Anexe a um servidor novacode em execução (por exemplo, http://localhost:4096)     |
| <nobr><code>{"--password"}</code></nobr> | `-p`  | Senha de autenticação básica (padrão: `OPENCODE_SERVER_PASSWORD`)                 |
| <nobr><code>{"--username"}</code></nobr> | `-u`  | Usuário de autenticação básica (padrão: `OPENCODE_SERVER_USERNAME` ou `novacode`) |
...
Inicie um servidor novacode sem cabeça para acesso à API. Confira a [documentação do servidor](/docs/server) para a interface HTTP completa.

```bash
novacode serve
```

Isso inicia um servidor HTTP que fornece acesso à funcionalidade do novacode sem a interface TUI. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `novacode`).
...
Liste todas as sessões do novacode.
...
Mostre o uso de tokens e estatísticas de custo para suas sessões do novacode.
...
Importe dados da sessão de um arquivo JSON ou URL de compartilhamento do novacode.

```bash
novacode import <file>
```

Você pode importar de um arquivo local ou de uma URL de compartilhamento do novacode.
...
Inicie um servidor novacode sem cabeça com uma interface web.

```bash
novacode web
```

Isso inicia um servidor HTTP e abre um navegador para acessar o novacode através de uma interface web. Defina `OPENCODE_SERVER_PASSWORD` para habilitar a autenticação básica HTTP (o nome de usuário padrão é `novacode`).
...
Desinstale o novacode e remova todos os arquivos relacionados.
...
Atualiza o novacode para a versão mais recente ou uma versão específica.
...
A CLI do novacode aceita as seguintes flags globais.
...
O novacode pode ser configurado usando variáveis de ambiente.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Substituir nome de usuário de autenticação básica (padrão `novacode`) |
