...
O NovaCode Go é uma assinatura de baixo custo de **US$ 10/mês** que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no NovaCode. Você assina o NovaCode Go e
...
O NovaCode Go lhe dá acesso a esses modelos por **US$ 10/mês**.

---

## Como funciona

O NovaCode Go funciona como qualquer outro provedor no NovaCode.

1. Você entra no **<a href={console}>NovaCode Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `NovaCode Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o NovaCode Go.
:::
...
O NovaCode Go foi projetado para o [NovaCode](https://novacode.ai) e outros agentes de programação
...
3. Enviar um ID de sessão estável em `x-novacode-session` para cada conversa, para que possamos otimizar o roteamento e
   o cache de prompts.

### Clientes validados

Além do NovaCode, os clientes a seguir foram validados para funcionar corretamente
com o NovaCode Go. No entanto, não garantimos que continuarão funcionando no futuro.

| Cliente           | Suporte a sessões                                                                                                                                                                                                                                                     |
| ----------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Hermes**        | As builds que contêm o [PR #101864](https://github.com/NousResearch/hermes-agent/pull/101864) enviam o header nas requisições principais e auxiliares do NovaCode. A correção foi incorporada após a v0.21.0; essa versão por si só não a inclui.                     |
...
| **ZCode**         | O Go reconhece seu header de sessão nativo. Nossa [solicitação de `x-novacode-session`](https://github.com/zai-org/feedback/issues/492) continua aberta, mas não é mais necessário enviar especificamente esse header.                                                |
| **Pi**            | As builds atuais enviam informações de sessão para o NovaCode. Atualize instalações mais antigas.                                                                                                                                                                     |
...
| **Kilo Code CLI** | As builds que contêm o [PR #13752](https://github.com/Kilo-Org/kilocode/pull/13752) restauram os headers de sessão do NovaCode. Essa correção abrange a CLI, não a extensão do VS Code. Consulte a [issue #13723](https://github.com/Kilo-Org/kilocode/issues/13723). |
...
O [ID do modelo](/docs/config/#models) na sua configuração do NovaCode
usa o formato `novacode-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`novacode-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o NovaCode também.
