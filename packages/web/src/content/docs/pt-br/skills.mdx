...
As habilidades do agente permitem que o novacode descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O novacode pesquisa nesses locais:

- Configuração do projeto: `.novacode/skills/<name>/SKILL.md`
- Configuração global: `~/.config/novacode/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o novacode sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.novacode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.novacode/skills/git-release/SKILL.md` assim:
...
O novacode lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `novacode.json`:
...
**Para agentes embutidos** (em `novacode.json`):
