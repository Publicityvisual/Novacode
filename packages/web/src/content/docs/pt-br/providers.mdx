---
title: Provedores
description: Usando qualquer provedor LLM no novacode.
---

import config from "../../../../config.mjs"
export const console = config.console

novacode usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do novacode.

---

### Credenciais

Quando você adiciona as chaves da API de um provedor com o comando `/connect`, elas são armazenadas em `~/.local/share/novacode/auth.json`.

---

### Configuração

Você pode personalizar os provedores através da seção `provider` na sua configuração do novacode.
...
NovaCode Zen é uma lista de modelos fornecidos pela equipe do novacode que foram testados e verificados para funcionar bem com o novacode. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o NovaCode Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `NovaCode Zen` e acesse [novacode.ai/zen](https://novacode.ai/zen).
...
Funciona como qualquer outro provedor no novacode e é completamente opcional.

---

## NovaCode Go

NovaCode Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do novacode que foram
testados e verificados para funcionar bem com o novacode.

1. Execute o comando `/connect` no TUI, selecione `NovaCode Go` e acesse [novacode.ai/zen](https://novacode.ai/zen).
...
Para usar o Amazon Bedrock com o novacode:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o novacode:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `novacode.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no novacode não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o novacode para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o novacode funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do novacode.
...
armazenar o token no armazenamento de autenticação do novacode.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
novacode usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o novacode
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `novacode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth).

##### Configuração

Personalize através do `novacode.json`:
...
Para usar sua assinatura do GitHub Copilot com o novacode:
...
Para usar o Google Vertex AI com o novacode:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o novacode.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do novacode, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session) para registrar automaticamente cada conversa do novacode como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do novacode listada como uma sessão separada.
...
Você pode configurar o novacode para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o novacode para usar modelos locais através do LM Studio.
...
Você pode configurar o novacode para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o novacode. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/novacode) para detalhes.
:::
...
Para usar o Ollama Cloud com o novacode:
...
3. Copie a chave da API para uso no novacode.
...
6. **Importante**: Antes de usar modelos em nuvem no novacode, você deve puxar as informações do modelo localmente:
...
NovaCode Zen é uma lista de modelos testados e verificados fornecidos pela equipe do novacode. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>NovaCode Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **NovaCode Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do novacode.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do novacode.
...
5. Você também pode personalizá-los através da sua configuração do novacode. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o novacode:
...
Você também pode personalizar modelos através da sua configuração do novacode. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o novacode. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no novacode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `novacode.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o novacode entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `novacode auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do novacode e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do novacode.
