---
title: Suporte ACP
description: Use o novacode em qualquer editor compatível com ACP.
---

O novacode suporta o [Agent Client Protocol](https://agentclientprotocol.com) ou (ACP), permitindo que você o utilize diretamente em editores e IDEs compatíveis.
...
Para usar novacode via ACP, configure seu editor para executar o comando `novacode acp`.

Este comando inicia o novacode como um subprocesso compatível com ACP que se comunica com seu editor via JSON-RPC através do stdio.
...
Instale o NovaCode pelo [Registro ACP do Zed](https://zed.dev/docs/ai/external-agents#registry) executando `zed: acp registry` na Paleta de Comandos.

Para usar um executável personalizado do NovaCode, adicione-o à configuração do [Zed](https://zed.dev) (`~/.config/zed/settings.json`):
...
Para abri-lo, use o novo agente 'novacode' no seletor de agentes do AI Chat.
...
Para usar novacode como um agente ACP no [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), adicione o seguinte à sua configuração do Neovim:
...
Esta configuração configura o CodeCompanion para usar novacode como o agente ACP para chat.
...
O novacode funciona da mesma forma via ACP como funciona no terminal. Todos os recursos são suportados:
...
- Servidores MCP configurados na sua configuração do novacode
