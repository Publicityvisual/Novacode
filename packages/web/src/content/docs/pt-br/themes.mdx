...
Com o novacode, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o novacode usa nosso próprio tema `novacode`.
...
O novacode vem com vários temas embutidos.
...
- Querem que o novacode corresponda à aparência do seu terminal
...
O novacode suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/novacode/themes/*.json` ou `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.novacode/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.novacode/themes/*.json`
