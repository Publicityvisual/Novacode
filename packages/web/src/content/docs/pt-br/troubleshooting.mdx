...
Para depurar problemas com o novacode, comece verificando os logs e os dados locais que ele armazena no disco.
...
Você pode definir o nível de log com a opção de linha de comando `--log-level` para obter informações de depuração mais detalhadas. Por exemplo, `novacode --log-level DEBUG`.

---

## Armazenamento

O novacode armazena dados de sessão e outros dados do aplicativo no disco em:
...
O novacode Desktop executa um servidor novacode local (o sidecar `novacode-cli`) em segundo plano. A maioria dos problemas é causada por um plugin com mau funcionamento, um cache corrompido ou uma configuração de servidor incorreta.
...
- Apenas macOS: menu `NovaCode` -> **Recarregar Webview** (ajuda se a interface estiver em branco/congelada).
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (ou `~/.config/novacode/novacode.json`)
- **macOS/Linux** (instalações mais antigas): `~/.local/share/novacode/novacode.jsonc`
...
O novacode também pode carregar plugins locais do disco. Mova-os temporariamente para fora do caminho (ou renomeie a pasta) e reinicie o aplicativo de desktop:
...
Se desativar plugins não ajudar (ou se a instalação de um plugin estiver travada), limpe o cache para que o novacode possa reconstruí-lo.

1. Saia completamente do novacode Desktop.
2. Exclua o diretório de cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> cole `~/.cache/novacode`
- **Linux**: exclua `~/.cache/novacode` (ou execute `rm -rf ~/.cache/novacode`)
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.cache\novacode`

3. Reinicie o novacode Desktop.

---

### Corrigindo problemas de conexão com o servidor

O novacode Desktop pode iniciar seu próprio servidor local (padrão) ou conectar-se a uma URL de servidor que você configurou.
...
Se seu `novacode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
...
No Windows, o novacode Desktop requer o **WebView2 Runtime** do Microsoft Edge. Se o aplicativo abrir em uma janela em branco ou não iniciar, instale/atualize o WebView2 e tente novamente.

---

### Windows: Problemas gerais de desempenho

Se você estiver enfrentando desempenho lento, problemas de acesso a arquivos ou problemas no terminal no Windows, tente usar [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). O WSL fornece um ambiente Linux que funciona de forma mais integrada com os recursos do novacode.

---

### Notificações não aparecendo

O novacode Desktop só mostra notificações do sistema quando:

- as notificações estão habilitadas para o novacode nas configurações do seu sistema operacional, e
...
1. Saia do novacode Desktop.
2. Encontre e exclua estes arquivos (eles estão no diretório de dados do aplicativo novacode Desktop):

- `novacode.settings.dat` (URL do servidor padrão do desktop)
- `novacode.global.dat` e `novacode.workspace.*.dat` (estado da interface como servidores/projetos recentes)
...
Se você estiver enfrentando problemas com o novacode:
...
Para descobrir quais modelos você tem acesso, execute `novacode models`
...
Se você encontrar erros de chamada de API, isso pode ser devido a pacotes de provedores desatualizados. O novacode instala dinamicamente pacotes de provedores (OpenAI, Anthropic, Google, etc.) conforme necessário e os armazena em cache localmente.
...
2. Reinicie o novacode para reinstalar os pacotes de provedores mais recentes

Isso forçará o novacode a baixar as versões mais recentes dos pacotes de provedores, o que muitas vezes resolve problemas de compatibilidade com parâmetros de modelo e alterações na API.
...
O novacode detectará se você está usando Wayland e preferirá `wl-clipboard`, caso contrário, tentará encontrar ferramentas de área de transferência na ordem: `xclip` e `xsel`.
