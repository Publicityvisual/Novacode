...
Existem dois tipos de agentes no novacode; agentes primários e subagentes.
...
novacode vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
NovaCode vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

NovaCode vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do NovaCode, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `novacode.json`:
...
- Global: `~/.config/novacode/agents/`
- Por projeto: `.novacode/agents/`
...
Se nenhuma temperatura for especificada, o novacode usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do novacode quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do novacode usa o formato `provider/model-id`. Por exemplo, se você estiver usando [NovaCode Zen](/docs/zen), você usaria `novacode/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Execute `novacode models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/novacode).
:::
