---
title: Plugins
description: Escreva seus próprios plugins para estender o novacode.
---

Plugins permitem que você estenda o novacode conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do novacode.
...
- `.novacode/plugins/` - Plugins em nível de projeto
- `~/.config/novacode/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/novacode/node_modules/`.
...
1. Configuração global (`~/.config/novacode/novacode.json`)
2. Configuração do projeto (`novacode.json`)
3. Diretório de plugins global (`~/.config/novacode/plugins/`)
4. Diretório de plugins do projeto (`.novacode/plugins/`)
...
O novacode executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do novacode para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o novacode.
...
:::note
Se você estiver usando o aplicativo desktop novacode, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o novacode de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao novacode:
...
O helper `tool` cria uma ferramenta personalizada que o novacode pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o novacode junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://novacode.ai/docs/sdk) para detalhes.
