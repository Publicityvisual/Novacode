...
`.novacode/commands/test.md` を作成します。
...
カスタムコマンドは、NovaCode 設定を通じて、または `commands/` ディレクトリに Markdown ファイルを作成することによって追加できます。

---

### JSON

NovaCode [設定](/docs/config)で `command` オプションを使用します:
...
- グローバル: `~/.config/novacode/commands/`
- プロジェクトごと: `.novacode/commands/`
...
NovaCode には、`/init`、`/undo`、`/redo`、`/share`、`/help` などのいくつかの組み込みコマンドが含まれています。 [詳細はこちら](/docs/tui#commands)。
