...
NovaCode には、`tui.json` を通じてカスタマイズできるキーバインドのリストがあります。
...
NovaCode は、ほとんどのキーバインドに `leader` キーを使用します。これにより、ターミナルでの競合が回避されます。
...
NovaCode デスクトップアプリのプロンプト入力は、テキストを編集するための一般的な Readline/Emacs スタイルのショートカットをサポートしています。これらは組み込みであり、現在 `novacode.json` を介して構成することはできません。
