---
title: LSPサーバー
description: NovaCode は LSP サーバーと統合します。
---

NovaCode は Language Server Protocol (LSP) サーバーと統合し、診断を agent へのフィードバックとして使用できます。

---

## 組み込み

NovaCode には、一般的な言語用のいくつかの組み込み LSP サーバーが付属しています。
...
LSP が有効で NovaCode がファイルを開くと、次のようになります。
...
NovaCode 設定の `lsp` セクションを通じて LSP サーバーを有効化し、カスタマイズできます。
