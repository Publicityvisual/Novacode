...
Agent skills omogucavaju NovaCode da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
NovaCode pretrazuje ove lokacije:

- Konfiguracija projekta: `.novacode/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/novacode/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, NovaCode ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.novacode/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/novacode/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.novacode/skills/git-release/SKILL.md` ovako:
...
NovaCode navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `novacode.json`:
...
**Za ugradene agente** (u `novacode.json`):
