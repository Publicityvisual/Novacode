...
U NovaCode mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, NovaCode koristi nasu `novacode` temu.
...
NovaCode dolazi sa vise ugradenih tema.
...
- Zele da NovaCode odgovara izgledu njihovog terminala
...
NovaCode podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/novacode/themes/*.json` ili `$XDG_CONFIG_HOME/novacode/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.novacode/themes/*.json`
4. **Trenutni radni direktorij** - `./.novacode/themes/*.json`
