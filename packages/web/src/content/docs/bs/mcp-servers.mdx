...
Možete dodati vanjske alate u NovaCode koristeći _Model Context Protocol_, ili MCP. NovaCode podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [NovaCode Config](https://novacode.ai/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/novacode`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
NovaCode automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, NovaCode će:
...
Ako server zahtijeva autentifikaciju, NovaCode će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `novacode mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, NovaCode sigurno čuva tokene u `~/.local/share/novacode/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u NovaCode, zajedno s ugrađenim alatima. Možete njima upravljati kroz NovaCode konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete NovaCode sa vašim Sentry nalogom.
