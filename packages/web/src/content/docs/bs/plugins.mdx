---
title: Dodaci
description: Napišite vlastite dodatke za proširenje NovaCode.
---

Dodaci vam omogućavaju da proširite NovaCode spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja NovaCode.
...
- `.novacode/plugins/` - Dodaci na nivou projekta
- `~/.config/novacode/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/novacode/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/novacode/novacode.json`)
2. Konfiguracija projekta (`novacode.json`)
3. Globalni direktorij dodataka (`~/.config/novacode/plugins/`)
4. Direktorij dodataka projekta (`.novacode/plugins/`)
...
NovaCode pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje NovaCode.
...
Spriječite novacode da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u novacode:
...
Pomoćnik `tool` kreira prilagođeni alat koji novacode može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za novacode zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://novacode.ai/docs/sdk) za detalje.
