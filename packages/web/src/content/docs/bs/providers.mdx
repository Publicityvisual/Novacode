---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u NovaCode.
---

import config from "../../../../config.mjs"
export const console = config.console

NovaCode koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj NovaCode konfiguraciji.
...
u `~/.local/share/novacode/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem NovaCode
config.
...
NovaCode Zen je lista modela koje je obezbedio NovaCode tim koji su bili
testirano i potvrđeno da dobro radi sa NovaCode. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa NovaCode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite novacode i idite na [novacode.ai/auth](https://novacode.ai/auth).
...
Radi kao i svaki drugi provajder u NovaCode i potpuno je opcionalan za korištenje.

---

## NovaCode Go

NovaCode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža NovaCode tim i koji su testirani i verificirani da dobro rade s NovaCode-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `NovaCode Go` i idite na [novacode.ai/auth](https://novacode.ai/zen).
...
Da biste koristili Amazon Bedrock s NovaCode:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete novacode:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `novacode.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u NovaCode.
:::
...
Možete konfigurirati novacode za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi novacode ispravno radio.
   :::
...
Također možete dodati modele kroz svoju novacode konfiguraciju.
...
da pohrani token u novacode auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
NovaCode koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate NovaCode
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`novacode.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [novacode-gitlab-auth](https://www.npmjs.com/package/novacode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `novacode.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s novacode:
...
Za korištenje Google Vertex AI s NovaCode:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće novacode.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem novacodea, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [novacode-helicone-session](https://github.com/H2Shami/novacode-helicone-session) da automatski prijavite svaki NovaCode razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki NovaCode razgovor naveden kao zasebna sesija.
...
Možete konfigurirati novacode za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati novacode za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati novacode za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za NovaCode. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/novacode) za detalje.
:::
...
Da biste koristili Ollama Cloud s NovaCode:
...
3. Kopirajte API ključ za korištenje u NovaCode.
...
6. **Važno**: Prije upotrebe modela oblaka u NovaCode, morate lokalno povući informacije o modelu:
...
NovaCode Zen je lista testiranih i verifikovanih modela koju je obezbedio NovaCode tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>NovaCode Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **NovaCode Zen**.

```txt
   /connect
```

3. Unesite svoj NovaCode API ključ.
...
Također možete dodati dodatne modele putem vaše novacode konfiguracije.
...
5. Također ih možete prilagoditi putem vaše novacode konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju novacode konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s novacode-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `novacode.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju NovaCode da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `novacode auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite NovaCode konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj novacode konfiguraciji.
