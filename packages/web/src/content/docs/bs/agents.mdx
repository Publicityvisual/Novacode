...
Postoje dvije vrste agenata u NovaCode: primarni agenti i podagenti.
...
NovaCode dolazi sa dva ugrađena primarna agenta, **Build** i **Plan**. Pogledat ćemo ih u nastavku.
...
NovaCode dolazi sa tri ugrađena subagenta, **General**, **Explore** i **Scout**. Ovo ćemo pogledati u nastavku.

---

## Ugrađeni

NovaCode dolazi sa dva ugrađena primarna agenta i tri ugrađena subagenta.
...
Agent samo za čitanje za istraživanje eksterne dokumentacije i zavisnosti. Koristite ga kada trebate klonirati repozitorij zavisnosti u NovaCode-ov upravljani cache, pregledati izvorni kod biblioteke ili uporediti lokalni kod sa upstream implementacijama bez mijenjanja vašeg radnog prostora.
...
Konfigurirajte agente u svom konfiguracijskom fajlu `novacode.json`:
...
- Globalno: `~/.config/novacode/agents/`
- Po projektu: `.novacode/agents/`

```markdown title="~/.config/novacode/agents/review.md"
---
...
Ako temperatura nije navedena, NovaCode koristi standardne postavke specifične za model; obično 0 za većinu modela, 0.55 za Qwen modele.
...
Ova putanja je relativna u odnosu na mjesto gdje se nalazi konfiguracijski fajl. Dakle, ovo radi i za globalnu NovaCode konfiguraciju i za konfiguraciju specifične za projekat.
...
ID modela u vašoj NovaCode konfiguraciji koristi format `provider/model-id`. Na primjer, ako koristite [NovaCode Zen](/docs/zen), koristili biste `novacode/gpt-5.1-codex` za GPT 5.1 Codex.
...
```json title="novacode.json" {3-6,9-12}
{
  "$schema": "https://novacode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="novacode.json" {3-5,8-10}
{
  "$schema": "https://novacode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Pokrenite `novacode models` da vidite listu dostupnih modela.
:::
...
:::tip
Imate li agenta kojeg biste željeli podijeliti? [Pošalji PR](https://github.com/anomalyco/novacode).
:::
...
```markdown title="~/.config/novacode/agents/docs-writer.md"
---
...
```markdown title="~/.config/novacode/agents/security-auditor.md"
---
