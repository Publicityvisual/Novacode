...
Da biste otklonili probleme s NovaCode, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `novacode --log-level DEBUG`.

---

## Pohrana

NovaCode pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:
...
NovaCode Desktop pokreće lokalni NovaCode server (`novacode-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `NovaCode` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/novacode/novacode.jsonc` (ili `~/.config/novacode/novacode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/novacode/novacode.jsonc`
...
NovaCode također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga NovaCode mogao ponovo izgraditi.

1. Potpuno zatvorite NovaCode Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/novacode`
- **Linux**: obrišite `~/.cache/novacode` (ili pokrenite `rm -rf ~/.cache/novacode`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\novacode`

3. Ponovo pokrenite NovaCode Desktop.

---

### Rješavanje problema sa vezom na serveru

NovaCode Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `novacode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
...
Na Windows-u, NovaCode Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa NovaCode karakteristikama.

---

### Obavještenja se ne prikazuju

NovaCode Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za NovaCode u postavkama vašeg OS-a, i
...
1. Zatvorite NovaCode Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka NovaCode Desktop aplikacije):

- `novacode.settings.dat` (URL zadanog servera za desktop)
- `novacode.global.dat` i `novacode.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `novacode models`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. NovaCode dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
2. Ponovo pokrenite NovaCode da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti NovaCode da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
NovaCode će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
