---
title: Server
description: Komunicirajte s novacode serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `novacode serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji novacode klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `novacode.local` |
...
Postavite `OPENCODE_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `novacode`, ili postavite `OPENCODE_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `novacode serve` i za `novacode web`.
...
Kada pokrenete `novacode`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite novacode server za programsku interakciju sa novacode-om.
:::

Ova arhitektura omogucava novacode podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `novacode serve` da startate standalone server. Ako je novacode TUI vec pokrenut, `novacode serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste NovaCode [IDE](/docs/ide) pluginovi.
...
novacode server izlaže sljedece API-je.
