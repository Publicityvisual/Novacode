---
title: CLI
description: NovaCode CLI opcije i naredbe.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

NovaCode CLI po defaultu pokreće [TUI](/docs/tui) kada se pokrene bez ikakvih argumenata.

```bash
novacode
```

Ali takođe prihvata naredbe kao što je dokumentovano na ovoj stranici. Ovo vam omogućava programsku interakciju sa NovaCode.
...
Pokrenite NovaCode terminalski korisnički interfejs.
...
NovaCode CLI takođe ima sljedeće naredbe.

---

### agent

Upravljajte NovaCode agentima.
...
Priključite terminal na već pokrenut NovaCode backend server pokrenut putem `serve` ili `web` naredbi.

```bash
novacode attach [url]
```

Ovo omogućava korištenje TUI-ja sa udaljenim NovaCode backend-om. Na primjer:
...
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `novacode`) |
...
NovaCode pokreće lista provajdera na [Models.dev](https://models.dev), tako da možete koristiti `novacode auth login` da konfigurirate API ključeve za bilo kojeg provajdera kojeg želite koristiti. Ovo je pohranjeno u `~/.local/share/novacode/auth.json`.

```bash
novacode auth login
```

Kada se NovaCode pokrene, učitava dobavljače iz datoteke vjerodajnica. I ako postoje neki ključevi definirani u vašim okruženjima ili `.env` fajl u vašem projektu.
...
Koristite `--refresh` zastavicu da ažurirate keširanu listu modela. Ovo je korisno kada su novi modeli dodani provajderu i želite da ih vidite u NovaCode.
...
Pokrenite NovaCode u neinteraktivnom modu tako što ćete direktno proslijediti prompt.
...
Također možete priključiti pokrenutu `novacode serve` instancu kako biste izbjegli vrijeme hladnog pokretanja MCP servera pri svakom pokretanju:
...
| <nobr><code>{"--attach"}</code></nobr>   |        | Priključite na pokrenuti NovaCode server (npr. http://localhost:4096)                         |
| <nobr><code>{"--password"}</code></nobr> | `-p`   | Lozinka za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_PASSWORD`)                       |
| <nobr><code>{"--username"}</code></nobr> | `-u`   | Korisničko ime za osnovnu autentifikaciju (zadano: `OPENCODE_SERVER_USERNAME` ili `novacode`) |
...
Pokrenite NovaCode headless server za API pristup. Pogledajte [server docs](/docs/server) za kompletan HTTP interfejs.

```bash
novacode serve
```

Ovo pokreće HTTP server koji pruža API pristup funkcionalnosti NovaCode-a bez TUI interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `novacode`).
...
Prikaži statistiku upotrebe tokena i troškova za vaše NovaCode sesije.
...
Uvezite podatke sesije iz JSON datoteke ili NovaCode dijeljenog URL-a.

```bash
novacode import <file>
```

Možete uvesti iz lokalne datoteke ili NovaCode dijeljenog URL-a.
...
Pokrenite NovaCode headless server sa web interfejsom.

```bash
novacode web
```

Ovo pokreće HTTP server i otvara web pretraživač za pristup NovaCode-u preko web interfejsa. Postavite `OPENCODE_SERVER_PASSWORD` da omogućite HTTP osnovnu auth (korisničko ime je zadano na `novacode`).
...
Deinstalirajte NovaCode i uklonite sve povezane datoteke.
...
Ažurira NovaCode na najnoviju verziju ili određenu verziju.
...
NovaCode CLI prihvata sljedeće globalne zastavice.
...
NovaCode se može konfigurirati pomoću varijabli okruženja.
...
| `OPENCODE_SERVER_USERNAME`            | string  | Poništi osnovno korisničko ime autentifikacije (zadano `novacode`) |
