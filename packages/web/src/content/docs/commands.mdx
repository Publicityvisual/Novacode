...
Create `.novacode/commands/test.md`:
...
You can add custom commands through the NovaCode config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your NovaCode [config](/docs/config):
...
novacode includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
