...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve novacode route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "novacode")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "novacode")).toBe("openai")
    expect(statProvider("big-pickle", "", "novacode")).toBe("unknown")
  ...
  test("attributes hy4 preview traffic to Tencent instead of the unknown provider", () => {
    ...
    expect(toModelAggregate(aggregate("hy4-preview", "novacode"))).toMatchObject([
      { model: "hy4-preview", provider: "tencent" },
    ])
    expect(toProviderAggregate(aggregate("hy4-preview", "novacode"))).toMatchObject([{ provider: "tencent" }])
    expect(toGeoAggregate({ ...aggregate("hy4-preview", "novacode"), country: "US" })).toMatchObject([
      { model: "hy4-preview", provider: "tencent" },
    ])
  })
  ...
  test("keeps stealth model usage without exposing the route provider", () => {
    ...
    ;["novacode-go/union-alpha", "novacode/union-alpha"].forEach((model) => {
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "novacode"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep novacode as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "novacode"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "novacode"))).toMatchObject([{ provider: "unknown" }])
    expect(toProviderAggregate(aggregate("muse-spark-1.2-contributor", "unknown"))).toMatchObject([
      { provider: "meta" },
    ])
  })

  test("geo aggregates never keep novacode or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "novacode"), country: "US" })).toMatchObject([
  ...
  test("builds bounded R2 SQL queries for each day and week", () => {
    ...
    queries.forEach((query) => {
      ...
      expect(query).toContain("= 'novacode-go/union-alpha' THEN 'union-alpha'")
      expect(query).toContain("= 'novacode/union-alpha' THEN 'union-alpha'")
  ...
  test("builds complete week-over-week retention queries", () => {
    ...
    expect(queries[0]?.query).toContain("= 'novacode-go/union-alpha' THEN 'union-alpha'")
    expect(queries[0]?.query).toContain("= 'novacode/union-alpha' THEN 'union-alpha'")
