...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("novacode-find")
  api.delete("novacode-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("novacode-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("novacode-find", new Highlight(...rest))
    return true
  }
