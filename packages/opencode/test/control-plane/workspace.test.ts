...
import { WorkspaceTable } from "@novacode-ai/core/control-plane/workspace.sql"
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@novacode.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=novacode-test"
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=novacode-test")
