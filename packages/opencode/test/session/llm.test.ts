...
import { LLMClient, RequestExecutor } from "@novacode-ai/llm/route"
...
describe("session.llm.stream", () => {
  ...
  const novacodeFixture = { providerID: "novacode-test", modelID: vivgridFixture.modelID }

  it.instance(
    "sends the parent session header for novacode providers",
    () =>
      Effect.gen(function* () {
        ...
        const resolved = yield* Provider.use.getModel(
          ProviderV2.ID.make(novacodeFixture.providerID),
          ModelV2.ID.make(novacodeFixture.modelID),
        )
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(novacodeFixture.providerID),
            modelID: resolved.id,
          },
      ...
      }),
    {
      config: () => {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        return {
          enabled_providers: [novacodeFixture.providerID],
          provider: {
            [novacodeFixture.providerID]: {
              name: "NovaCode Test",
