...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "novacode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "novacode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novacode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "novacode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["novacode.json", "novacode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "novacode.json"),
        path.join(project, "novacode.jsonc"),
      ])
    })
  })
