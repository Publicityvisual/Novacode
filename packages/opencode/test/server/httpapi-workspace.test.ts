...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  headers.set("x-novacode-directory", directory)
    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://novacode.ai/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-novacode-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-novacode-workspace")
