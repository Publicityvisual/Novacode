// Subprocess integration tests for `novacode run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `novacode.run(message, opts?)` to spawn `bun src/index.ts run ...` with
...
describe("novacode run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* novacode.run("say hi")
        novacode.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* novacode.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* novacode.run("think", { extraArgs: ["--thinking"] })
        novacode.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* novacode.run("think again")
        novacode.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* novacode.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* novacode.run("say hi", { format: "json" })
        novacode.expectExit(result, 0)

        const events = novacode.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ novacode }) =>
      Effect.gen(function* () {
        const result = yield* novacode.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const result = yield* novacode.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        ...
        yield* llm.text("recovered")
        const result = yield* novacode.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* novacode.run("request permission", { permission: { bash: "ask" } })
        novacode.expectExit(denied, 0)
        ...
        const allowed = yield* novacode.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        novacode.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* novacode.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        novacode.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, novacode }) =>
      Effect.gen(function* () {
        ...
        const server = yield* novacode.serve()

        const result = yield* novacode.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, novacode }) =>
      Effect.gen(function* () {
        const result = yield* novacode.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, novacode }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* novacode.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
