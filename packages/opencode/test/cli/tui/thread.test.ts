...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ novacode }) =>
    Effect.gen(function* () {
      const result = yield* novacode.spawn(["--replay-limit", "10"])

      novacode.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ novacode }) =>
    Effect.gen(function* () {
      const result = yield* novacode.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ novacode }) =>
    Effect.gen(function* () {
      const result = yield* novacode.spawn(["--mini", "--port", "4096"])
