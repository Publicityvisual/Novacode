...
describe("novacode acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ novacode }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ novacode }))
        ...
        expect(initialized.agentInfo?.name).toBe("NovaCode")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ novacode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ novacode })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("novacode-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "novacode-login" })).toMatchObject({
          result: {},
        })
  ...
  cliIt.live(
    ...
    ({ novacode }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("novacode-login")
