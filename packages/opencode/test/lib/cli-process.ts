// Subprocess test harness for the novacode CLI. Spawns the real binary against
...
//
// Configuration flows through novacode's built-in test affordances:
//   - OPENCODE_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENCODE_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENCODE_DISABLE_PROJECT_CONFIG : skip walking up for novacode.json
...
//
// Today only `novacode.run` is fully wired. The shape supports adding more
// builders (`novacode.serve(opts)`, `novacode.acp(opts)`, `novacode.auth(...)`)
...
const novacodeRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(novacodeRoot, "src/index.ts")
...
// Typed equivalent of constructing argv for `novacode run`. New flags should
...
// `novacode serve` is a long-lived process — it never exits on its own.
...
// `novacode acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `novacode serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `novacode acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("novacode.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const startRun = Effect.fn("novacode.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("novacode.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "novacode server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `novacode serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("novacode.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const novacode: OpencodeCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, novacode })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `novacode.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "novacode") {
...
// every call site. Use this for any test that needs the novacode CLI fixture.
//
...
// resources (e.g. `novacode.serve`) without an extra `Effect.scoped` wrapper —
