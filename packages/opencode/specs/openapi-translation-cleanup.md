...
Trim `packages/novacode/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/novacode/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/novacode`.
- `[x]` `bun typecheck` from `packages/novacode`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/novacode/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/novacode`.
- `[x]` `bun dev generate > /tmp/novacode-openapi.json` from `packages/novacode`.
...
- Edit `packages/novacode/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/novacode-openapi.json` from `packages/novacode`.
...
- Check source schemas in files like `packages/novacode/src/session/schema.ts`, `packages/novacode/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/novacode/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/novacode/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
