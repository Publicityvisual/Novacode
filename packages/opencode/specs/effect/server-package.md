...
Practical reference for a future `packages/server` split after the novacode
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/novacode`.
...
Until enough shared service code lives outside `packages/novacode`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/novacode`

It should not import `packages/novacode` services while `packages/novacode`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on novacode-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/novacode` implementation details.
