...
Delete `packages/novacode/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/novacode/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/novacode/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/novacode`
- TUI plugin loader tests in `packages/novacode` if runtime plugin API wiring changed
