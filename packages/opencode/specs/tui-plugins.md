...
- Author package entrypoint is `@novacode-ai/plugin/tui`.
...
```json
{
  "$schema": "https://novacode.ai/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/novacode-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@novacode-ai/plugin/tui`.
- `@novacode-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@novacode-ai/plugin/tui"
...
- The value is a semver range checked against the running NovaCode version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.novacode` is absent, no check is performed (backward compatible).
...
- `novacode plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `novacode plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.novacode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.novacode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.novacode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.novacode` dir or `OPENCODE_CONFIG_DIR`, NovaCode installs `@novacode-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@novacode-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus NovaCode's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
NovaCode registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, NovaCode pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@novacode-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"novacode"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `novacode.default`.
...
- Local plugins persist installed themes under the local `.novacode/themes` area near the plugin config source.
...
- Local smoke plugin: `.novacode/plugins/tui-smoke.tsx`
- Local vim plugin: `.novacode/plugins/tui-vim.tsx`
- Local smoke config: `.novacode/tui.json`
- Local smoke theme: `.novacode/plugins/smoke-theme.json`
