...
import { NovaCodeHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for novacode serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(NovaCodeHttpApi)
...
export const PublicApi = NovaCodeHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "novacode",
    version: "1.0.0",
    description: "novacode api",
