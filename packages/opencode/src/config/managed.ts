...
// Keys injected by macOS/MDM into the managed plist that are not NovaCode config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/novacode"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "novacode")
    default:
      return "/etc/novacode"
  }
}
