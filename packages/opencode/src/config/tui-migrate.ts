...
const TUI_SCHEMA_URL = "https://novacode.ai/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from novacode.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const novacode = await novacodeFiles(input)
  for (const file of novacode) {
...
async function novacodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "novacode"),
    ...(await Filesystem.findUp(["novacode.json", "novacode.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "novacode"))
  }
