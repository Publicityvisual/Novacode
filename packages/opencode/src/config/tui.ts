...
import { isRecord } from "@novacode-ai/tui/util/record"
...
import { TuiKeybind } from "@novacode-ai/tui/config/keybind"
...
import { TuiConfig } from "@novacode-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@novacode/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old novacode.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.novacode`
  ...
  // 4. `.novacode` directories (and OPENCODE_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".novacode") || dir === Flag.OPENCODE_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".novacode") && dir !== Flag.OPENCODE_CONFIG_DIR) continue
