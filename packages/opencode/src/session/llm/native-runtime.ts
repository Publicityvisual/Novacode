...
import type { LLMClientShape } from "@novacode-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("novacode"))
    return { type: "unsupported", reason: "provider is not openai, novacode, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @novacode-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains novacode-owned. The native runtime only adapts
      // the @novacode-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
