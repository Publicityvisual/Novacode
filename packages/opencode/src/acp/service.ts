...
import type { AssistantMessage, Message, OpencodeClient, Session, SessionMessageResponse } from "@novacode-ai/sdk/v2"
...
export const AuthMethodID = "novacode-login"
...
export class Service extends Context.Service<Service, Interface>()("@novacode/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `novacode auth login` in the terminal",
      name: "Login with novacode",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, novacode provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const novacodeProvider = providers[ProviderV2.ID.make("novacode")]
  const novacodeModel = novacodeProvider ? Provider.sort(Object.values(novacodeProvider.models))[0] : undefined
  if (novacodeProvider && novacodeModel) return { providerID: novacodeProvider.id, modelID: novacodeModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "NovaCode service failure", service })
}
