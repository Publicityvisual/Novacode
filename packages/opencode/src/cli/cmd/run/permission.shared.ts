...
import type { PermissionRequest } from "@novacode-ai/sdk/v2"
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until NovaCode is restarted.`]
  }

  return [
    "This will allow the following patterns until NovaCode is restarted.",
