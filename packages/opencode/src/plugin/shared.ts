...
export const DEPRECATED_PLUGIN_PACKAGES = ["novacode-openai-codex-auth", "novacode-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, novacodeVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(novacodeVersion) || semver.major(novacodeVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.novacode
  if (typeof range !== "string") return
  if (!semver.satisfies(novacodeVersion, range)) {
    throw new Error(`Plugin requires novacode ${range} but running ${novacodeVersion}`)
  }
}
