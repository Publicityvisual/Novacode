...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@novacode-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@novacode-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@novacode-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@novacode-ai/tui/context/theme"
import { Global } from "@novacode-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@novacode-ai/core/util/flock"
import { Flag } from "@novacode-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@novacode-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@novacode-ai/core/v1/config/plugin"
import { createCommandShim } from "@novacode-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@novacode-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".novacode"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".novacode", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".novacode", "tui.json"),
  }
}
