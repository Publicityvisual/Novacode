...
import type { IntegrationOAuthMethodRegistration } from "@novacode-ai/plugin/v2/effect/integration"
import { define } from "@novacode-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@novacode-ai/sdk/v2/types"
...
const defaultServer = "https://novacode.ai/console"
const clientID = "novacode-cli"
...
export const OpencodePlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("OpencodePlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("novacode")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load NovaCode provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("novacode", (integration) => {
        integration.name = "NovaCode"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "novacode", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("novacode")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("novacode")
      ...
      const item = catalog.provider.get(ProviderV2.ID.novacode)
      if (!item) return
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("novacode")),
