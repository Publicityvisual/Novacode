...
Independently, every `novacode.json` should declare
`"$schema": "https://novacode.ai/config.json"` so the user's editor catches
...
| Scope                         | Path                                                                                                                      |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Project config                | `./novacode.json`, `./novacode.jsonc`, or `.novacode/novacode.json` (novacode walks up from the cwd to the worktree root) |
| Global config                 | `~/.config/novacode/novacode.json` or `~/.config/novacode/novacode.jsonc` (NOT `~/.novacode/`)                            |
| Project agents                | `.novacode/agent/<name>.md` or `.novacode/agents/<name>.md`                                                               |
| Global agents                 | `~/.config/novacode/agent(s)/<name>.md`                                                                                   |
| Project commands              | `.novacode/command/<name>.md` or `.novacode/commands/<name>.md`                                                           |
| Global commands               | `~/.config/novacode/command(s)/<name>.md`                                                                                 |
| Project skills                | `.novacode/skill(s)/<name>/SKILL.md`                                                                                      |
...
```json
{
  ...
  "plugin": [
    "novacode-gemini-auth",
    "novacode-foo@1.2.3",
    "./local-plugin.ts",
    ["novacode-bar", { "option": "value" }]
  ],
...
```
.novacode/skills/my-skill/SKILL.md
```
...
```
.novacode/agent/my-reviewer.md      OR     .novacode/agents/my-reviewer.md
```
...
```json
"plugin": [
  "novacode-gemini-auth",            // npm spec, latest
  "novacode-foo@1.2.3",              // npm spec, pinned
  "./local-plugin.ts",               // file path, relative to the declaring config
  "file:///abs/path/plugin.js",      // file URL
  ["novacode-bar", { "key": "val" }] // tuple form with options
]
```

Auto-discovered plugins (no config entry needed): any `*.ts` or `*.js` file in
`.novacode/plugin/` or `.novacode/plugins/`.
...
```ts
import type { Plugin } from "@novacode-ai/plugin"
...
- `OPENCODE_DISABLE_PROJECT_CONFIG=1`: skip the project's local `novacode.json`
  and start from globals only. Run from the project directory, novacode loads,
  the user edits the broken file, then they restart without the flag.
- `OPENCODE_CONFIG=/path/to/file.json`: load an additional explicit config.
- `OPENCODE_CONFIG_CONTENT='{"$schema":"https://novacode.ai/config.json"}'`:
...
- Validate against the schema before writing. If you are unsure of a field's
  ...
  `https://novacode.ai/config.json` and read the schema rather than guessing.
- Preserve `$schema` and any existing fields the user did not ask to change.
- For agent, command, skill, and plugin definitions, prefer creating new files
  in the correct location over inlining everything in `novacode.json`.
