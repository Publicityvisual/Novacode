...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@novacode/acme")).toBe("@novacode/acme")
    expect(Npm.sanitize("@novacode/acme@1.0.0")).toBe("@novacode/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/novacode/acme.git"
    const expected = win ? "acme@git+https_//github.com/novacode/acme.git" : spec
