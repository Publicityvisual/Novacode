...
import { GoogleVertexPlugin } from "@novacode-ai/core/plugin/provider/google-vertex"
      ...
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.novacode, (provider) => {
          provider.api = {
            ...
            url: "https://novacode.ai/zen/v1",
          }
        }),
      )
      yield* addPlugin()

      const provider = required(yield* catalog.provider.get(ProviderV2.ID.novacode))
