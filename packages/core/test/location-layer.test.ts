...
import { define } from "@novacode-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@novacode-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "novacode.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "novacode.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
