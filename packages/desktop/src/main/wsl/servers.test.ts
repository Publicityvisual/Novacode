...
test("clears cached distro probes when removing a WSL server", () => {
  ...
  ).toEqual({ distroProbes: {}, novacodeChecks: {} })
})
...
test("ignores stale background NovaCode checks after removing a WSL server", async () => {
  ...
  expect(controller.getState().novacodeChecks).toEqual({})
})
...
test("probes addable distros in parallel before checking NovaCode", async () => {
  ...
  expect(novacode).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(novacode).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().novacodeChecks)).toEqual(["Debian", "Ubuntu"])
})
...
test("does not check NovaCode in addable distros that cannot execute commands", async () => {
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(novacode).toEqual(["Debian"])
  expect(Object.keys(controller.getState().novacodeChecks)).toEqual(["Debian"])
})
