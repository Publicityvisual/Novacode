...
export async function spawnWslSidecar(
  ...
  const novacode = await resolveWslOpencode(distro)
  if (!novacode) throw new Error(nativeT("desktop.wsl.error.novacodeNotInstalled", { distro }))
  ...
  const script = [
    ...
    `exec ${shellEscape(novacode)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
