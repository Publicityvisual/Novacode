...
This is the living design and status document for `@novacode-ai/codemode` and its existing V2 NovaCode adapter.
...
`packages/novacode/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
