...
describe("server action referer", () => {
  test("preserves same-origin return locations", () => {
    const request = new Request("https://dev.novacode.ai/_server?id=action", {
      headers: { referer: "https://dev.novacode.ai/auth?next=%2Fconsole" },
    })
    ...
    expect(
      referers.map((referer) =>
        sanitizeServerActionRequest(
          new Request("https://dev.novacode.ai/_server?id=action", {
            headers: referer === undefined ? undefined : { referer },
          }),
        ).headers.get("referer"),
      ),
    ).toEqual(["https://dev.novacode.ai", "https://dev.novacode.ai", "https://dev.novacode.ai"])
  })

  test("does not change other routes", () => {
    const request = new Request("https://dev.novacode.ai/auth", {
