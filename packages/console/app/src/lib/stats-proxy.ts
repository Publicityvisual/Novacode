...
import { Resource, waitUntil } from "@novacode-ai/console-resource"
import { LOCALE_HEADER, cookie, localeFromRequest, route, tag } from "~/lib/language"

const dataPath = "/data"
const statsCacheParam = "__novacode_stats_locale"
...
export async function statsProxy(evt: APIEvent) {
  ...
  targetUrl.hostname = Resource.App.stage === "production" ? "stats.novacode.ai" : "stats.dev.novacode.ai"
  targetUrl.port = ""
...
function withStatsCacheStatus(response: Response, status: "HIT" | "MISS") {
  const headers = new Headers(response.headers)
  headers.set("x-novacode-stats-cache", status)
