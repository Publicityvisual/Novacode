...
describe("authorization redirect validation", () => {
  test("allows registered NovaCode callbacks", () => {
    expect(isAllowedAuthorizationRedirect("app", "https://novacode.ai/auth/callback")).toBe(true)
    expect(isAllowedAuthorizationRedirect("app", "https://dev.novacode.ai/auth/callback")).toBe(true)
  ...
  test("rejects unregistered clients and external redirects", () => {
    expect(isAllowedAuthorizationRedirect("other", "https://novacode.ai/auth/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://evil.example/callback")).toBe(false)
    expect(isAllowedAuthorizationRedirect("app", "https://novacode.ai.evil.example/callback")).toBe(false)
