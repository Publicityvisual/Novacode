import type { IntegrationMethod, IntegrationOauthConnectOutput } from "@novacode-ai/client/promise"
import { Button } from "@novacode-ai/ui/button"
import { useDialog } from "@novacode-ai/ui/context/dialog"
import { Dialog } from "@novacode-ai/ui/dialog"
import { Icon } from "@novacode-ai/ui/icon"
import { IconButton } from "@novacode-ai/ui/icon-button"
import { List, type ListRef } from "@novacode-ai/ui/list"
import { ProviderIcon } from "@novacode-ai/ui/provider-icon"
import { Spinner } from "@novacode-ai/ui/spinner"
import { Tag } from "@novacode-ai/ui/tag"
import { TextField } from "@novacode-ai/ui/text-field"
import { ButtonV2 } from "@novacode-ai/ui/v2/button-v2"
import { DialogBody, DialogHeader, DialogTitle, DialogV2 } from "@novacode-ai/ui/v2/dialog-v2"
import { TextInputV2 } from "@novacode-ai/ui/v2/text-input-v2"
...
function ProviderPicker(props: {
  ...
  const note = (id: string) => {
    ...
    if (id === "novacode-go") return language.t("dialog.provider.novacodeGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "novacode"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.novacode.tagline")}</div>
          </Show>
...
function ProviderPickerV2(props: {
  ...
  const featured = ["novacode", "novacode-go", "anthropic", "openai", "google", "openrouter", "vercel"]
                  ...
                  <For each={group.items()}>
                    {(provider) => (
                      ...
                      >
                        ...
                        <Show when={provider.id === "novacode" || provider.id === "novacode-go"}>
                          <span class="min-w-0 truncate font-[440] text-v2-text-text-muted">
                            {language.t(
                              provider.id === "novacode"
                                ? "dialog.provider.novacode.tagline"
                                : "dialog.provider.novacodeGo.tagline",
                            )}
                          </span>
    ...
    if (method.type === "oauth") {
      ...
      await serverSDK()
        ...
        .then((x) => {
          ...
          if (props.provider === "novacode" && platform.platform === "desktop") {
            const url = new URL(x.data.url)
            url.searchParams.set("client_id", "novacode-desktop")
            x.data.url = url.href
          }
  ...
  function ApiAuthView() {
    ...
    if (newLayout())
      return (
        <div class="flex flex-col gap-5 px-3 text-[13px] font-[440] leading-5 tracking-[-0.04px] text-v2-text-text-muted">
          <Show
            when={provider().id === "novacode"}
            fallback={language.t("provider.connect.apiKey.description", { provider: provider().name })}
          >
            <div class="flex flex-col gap-5">
              <div>{language.t("provider.connect.novacodeZen.line1")}</div>
              <div>{language.t("provider.connect.novacodeZen.line2")}</div>
              <div>
                {language.t("provider.connect.novacodeZen.visit.prefix")}
                <ExternalLink
                  href="https://novacode.ai/zen"
                  class="text-v2-text-text-base focus-visible:rounded-xs focus-visible:outline-2 focus-visible:outline-v2-border-border-focus"
                >
                  {language.t("provider.connect.novacodeZen.visit.link")}
                </ExternalLink>
                {language.t("provider.connect.novacodeZen.visit.suffix")}
              </div>
            </div>
          </Show>
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "novacode"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.novacodeZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.novacodeZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.novacodeZen.visit.prefix")}
