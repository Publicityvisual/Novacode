...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.novacodeComment &&
          (part.metadata.novacodeComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "novacode.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.novacode.desktop.beta\\novacode.global.dat",
          mime: "text/plain",
    ...
    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.novacode.desktop.beta\\novacode.global.dat",
    )
  })
  ...
  test("handles macOS paths correctly", () => {
    ...
    const result = buildRequestParts({
      ...
      messageID: "msg_mac_1",
      sessionID: "ses_mac_1",
      sessionDirectory: "/Users/kelvin/Projects/novacode",
    })
    ...
    if (filePart?.type === "file") {
      ...
      expect(() => new URL(filePart.url)).not.toThrow()
      // Should be a normal Unix path
      expect(filePart.url).toBe("file:///Users/kelvin/Projects/novacode/README.md")
    }
  })
