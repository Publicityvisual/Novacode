...
import { wslNovaCodeAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.novacodeChecks[item.config.distro]
          const novacodeAction = () => wslNovaCodeAction(check())
          const busy = () => wsl.data?.job?.kind === "install-novacode" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={novacodeAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installNovaCode(item.config.distro))}
                    >
