...
- `novacode dev web` proxies `https://app.novacode.ai`, so local UI/CSS changes will not show there.
- For local UI changes, run the backend and app dev servers separately.
- Backend (from `packages/novacode`): `bun run ./src/index.ts serve --port 4096`
