...
const directory = "C:/NovaCode/TerminalComposerFocus"
...
test.beforeEach(async ({ page }) => {
  await mockNovaCodeServer(page, {
    ...
    provider: {
      ...
      connected: ["novacode"],
      default: { providerID: "novacode", modelID: "test" },
    },
    sessions: [
      {
        id: sessionID,
...
function seedCachedTerminal(page: Page) {
  return page.addInitScript(
    ({ terminalKey, ptyID }) => {
      localStorage.setItem("novacode.global.dat:layout", JSON.stringify({ terminal: { height: 320, opened: true } }))
