import { Message, Model, Part, Session, SessionStatus, SnapshotFileDiff, UserMessage } from "@novacode-ai/sdk/v2"
import { SessionTurn } from "@novacode-ai/session-ui/session-turn"
import { SessionReview } from "@novacode-ai/session-ui/session-review"
import { DataProvider } from "@novacode-ai/session-ui/context"
import { FileComponentProvider } from "@novacode-ai/ui/context/file"
import { WorkerPoolProvider } from "@novacode-ai/ui/context/worker-pool"
import { createAsync, query, useParams } from "@solidjs/router"
import { createMemo, createSignal, ErrorBoundary, For, Match, Show, Switch } from "solid-js"
import { Share } from "~/core/share"
import { Logo, Mark } from "@novacode-ai/ui/logo"
import { IconButton } from "@novacode-ai/ui/icon-button"
import { ProviderIcon } from "@novacode-ai/ui/provider-icon"
import { iife } from "@novacode-ai/core/util/iife"
import { Binary } from "@novacode-ai/core/util/binary"
import { NamedError } from "@novacode-ai/core/util/error"
import { DateTime } from "luxon"
import { createStore } from "solid-js/store"
import NotFound from "../[...404]"
import { Tabs } from "@novacode-ai/ui/tabs"
import { MessageNav } from "@novacode-ai/session-ui/message-nav"
import { FileSSR } from "@novacode-ai/session-ui/file-ssr"
...
const ClientOnlyWorkerPoolProvider = clientOnly(() =>
  import("@novacode-ai/session-ui/pierre/worker").then((m) => ({
      ...
      <Show when={data()}>
        {(data) => {
          ...
          const ogImage = createMemo(() => {
            ...
            return `https://social-cards.sst.dev/novacode-share/${encodedTitle}.png?model=${modelParam}&version=${version}&id=${data().shareID}`
          })

          return (
            <>
              <Show when={info().title}>
                <Title>{info().title} | NovaCode</Title>
              </Show>
              <Meta name="description" content="novacode - The AI coding agent built for the terminal." />
                        ...
                        <div class="relative bg-background-stronger w-screen h-screen overflow-hidden flex flex-col">
                          <header class="h-12 px-6 py-2 flex items-center justify-between self-stretch bg-background-base border-b border-border-weak-base">
                            ...
                            <div class="flex gap-3 items-center">
                              <IconButton
                                as={"a"}
                                href="https://github.com/anomalyco/novacode"
                                target="_blank"
                                icon="github"
                                variant="ghost"
                              />
                              <IconButton
                                as={"a"}
                                href="https://novacode.ai/discord"
                                target="_blank"
                                icon="discord"
                                variant="ghost"
                              />
