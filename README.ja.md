...
# パッケージマネージャー
npm i -g novacode-ai@latest        # bun/pnpm/yarn でもOK
scoop install novacode             # Windows
choco install novacode             # Windows
brew install anomalyco/tap/novacode # macOS と Linux（推奨。常に最新）
brew install novacode              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S novacode            # Arch Linux (Stable)
paru -S novacode-bin               # Arch Linux (Latest from AUR)
mise use -g novacode               # どのOSでも
nix run nixpkgs#novacode           # または github:anomalyco/novacode で最新 dev ブランチ
```
...
NovaCode はデスクトップアプリとしても利用できます。[releases page](https://github.com/anomalyco/novacode/releases) から直接ダウンロードするか、[novacode.ai/download](https://novacode.ai/download) を利用してください。
...
4. `$HOME/.novacode/bin` - デフォルトのフォールバック
...
NovaCode には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://novacode.ai/docs/agents) の詳細はこちら。

### ドキュメント

NovaCode の設定については [**ドキュメント**](https://novacode.ai/docs) を参照してください。

### コントリビュート

NovaCode に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### NovaCode の上に構築する

NovaCode に関連するプロジェクトで、名前に "novacode"（例: "novacode-dashboard" や "novacode-mobile"）を含める場合は、そのプロジェクトが NovaCode チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/novacode) | [X.com](https://x.com/novacode)
