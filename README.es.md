...
npm i -g novacode-ai@latest        # o bun/pnpm/yarn
scoop install novacode             # Windows
choco install novacode             # Windows
brew install anomalyco/tap/novacode # macOS y Linux (recomendado, siempre al día)
brew install novacode              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S novacode            # Arch Linux (Stable)
paru -S novacode-bin               # Arch Linux (Latest from AUR)
mise use -g novacode               # cualquier sistema
nix run nixpkgs#novacode           # o github:anomalyco/novacode para la rama dev más reciente
```
...
NovaCode también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/anomalyco/novacode/releases) o desde [novacode.ai/download](https://novacode.ai/download).
...
4. `$HOME/.novacode/bin` - Alternativa por defecto
...
NovaCode incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://novacode.ai/docs/agents).

### Documentación

Para más información sobre cómo configurar NovaCode, [**ve a nuestra documentación**](https://novacode.ai/docs).

### Contribuir

Si te interesa contribuir a NovaCode, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en NovaCode

Si estás trabajando en un proyecto basado en NovaCode y usas "novacode" como parte del nombre, por ejemplo, "novacode-dashboard" u "novacode-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de NovaCode y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/novacode) | [X.com](https://x.com/novacode)
